...
- 현재 개발 워크플로우는 증분 migration을 쓰지 않는다. DB 구조 변경은 `db_init.sql`에 최종 상태로 반영하고, 개발 DB는 `make seed-all && make db-init && make apply-seeds && make upload-character-avatars`로 재구축한다.
