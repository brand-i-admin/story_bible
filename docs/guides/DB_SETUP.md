...
```bash
make seed-all && make db-init ENV=real && make apply-seeds ENV=real && make upload-character-avatars ENV=real
```
...
`db-init`은 파괴적이다. 기존 DB 를 DROP/CREATE 한다. real 에 실행할 때는 반드시
`ENV=real`을 붙였는지, 그리고 정말 초기화할 환경인지 확인한다.

### 8.3 첫 실행에서 bucket warning 이 나오는 이유

처음 `make db-init ENV=real`을 실행하면 이런 경고가 나올 수 있다.
...
5. `make -n db-init ENV=<env>`로 잘못된 DB URL 을 잡지 않는지 확인.
...
- `make -n db-init ENV=real`이 real/prod DB URL 을 가리킨다.
- real DB 에 `db_init.sql`과 seed 가 적용되어 홈 데이터가 로드된다.
...
바꾸지 않는다. Makefile 기본값은 dev 로 유지한다. 운영 DB 초기화는 파괴적이므로
명시적으로 `ENV=real`을 붙이게 하는 현재 구조가 안전하다.
