...
문서 계층: 이 문서가 푸시 알림 기능의 메인 문서다. FCM, APNs, Supabase Edge
Function, secret 계층의 원리는 [INFRA_GUIDE.md](INFRA_GUIDE.md)를 보고, 새
Supabase 환경 전체 구축 순서는 [DB_SETUP.md](DB_SETUP.md)를 본다.
...
### Android
- "앱 추가" → Android 아이콘.
- Package name: `android/app/build.gradle.kts` 의 `applicationId`.
...
4. **절대 git 에 커밋 금지** — 저장소 루트의 `firebase-admin-sdk*.json`은 `.gitignore` 대상이다.
5. Supabase에 반영할 때는 `.env.supabase.secrets.example`을 복사해
   `.env.supabase.secrets`를 만들고 `FIREBASE_SERVICE_ACCOUNT`에 JSON 전체를 한 줄로 넣는다.
...
3. **db_init.sql 적용** — `make db-init ENV=<env>` 또는 Dashboard → SQL Editor 에 `db_init.sql` 전체 붙여넣고 Run. 새 헬퍼/트리거/`dispatch_daily_exploration_push`/pg_cron 모두 자동 등록된다.

자동 발송 경로:
- 새 이야기/인물 승인 (admin → `approve_event_proposal`) → broadcast row INSERT → 트리거 → push
- 매주 월요일 KST 9시 → `pick_weekly_character()` (한 주 시작 동행 격려 push-only, 미션 딥링크 없음)
- 매주 수요일 KST 9시 → `dispatch_daily_exploration_push()` (이야기 탐험 안내 push-only, `/daily-exploration` 딥링크)
...
3. Supabase SQL Editor 에서 즉시 푸쉬 테스트:
   ...
   → 본인 휴대폰에 “한주도 잘 보내고 계신가요!?” 알림이 오면 통과.
