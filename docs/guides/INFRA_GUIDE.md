...
문서 계층: 이 문서가 인프라의 메인 문서다. 새 Supabase 환경을 실제로 만드는
체크리스트는 [DB_SETUP.md](DB_SETUP.md), 다른 컴퓨터/팀원에게 공유해야 할 로컬
환경 파일 기준은 [LOCAL_ENV_FILES.md](LOCAL_ENV_FILES.md)를 본다. 푸시 알림만
세팅하거나 점검할 때는 [PUSH_SETUP.md](PUSH_SETUP.md)가 기능별 메인 문서다.
