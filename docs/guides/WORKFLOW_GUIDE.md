...
flowchart TB
    ...
    subgraph STATE["StoryState (Riverpod, 부팅 시 한 번 로드)"]
        ...
        S4[events of era]
        S5[selectedEraId]
        S6[selectedCharacterCodes]
        S7[displayedEventIds]
    end
    ...
    style DBL fill:#E8F5E9,stroke:#2E7D32
...
sequenceDiagram
    ...
    participant Map as StoryMapPanel
    ...
    U->>App: 아브라함 체크
    App->>State: toggleCharacter("abraham")
    Note over State: 영역 폴리곤 + 랜드마크는 그대로 유지

    U->>App: Step3 "다음" — 사건 5개 선택
...
### 5.5.4 콘텐츠 수정 흐름 (사용자 작업 절차)

```
랜드마크 추가/수정:
  ① assets/landmarks/landmarks.json 편집
     · code 는 unique, lat/lng/emoji 필수
     · era_codes 배열에 노출되고 싶은 시대 코드 추가 (eras.code 기준)
       ex) 예루살렘 성전 → ["era_monarchy", "era_exile_return", "era_nt_public_ministry"]
...
이벤트 좌표 ↔ 랜드마크 정렬 (모리아·헤브론·막벨라 등 어긋남 보정):
  ...
  ⚠️ 매핑 키워드는 tools/seed/align_events_to_landmarks.py 의 KEYWORD_TO_LANDMARK
     리스트로 제어. 새 landmark 추가 시 키워드도 함께 등록 권장.
```

⚠️ **stale 정리**: 빌더는 JSON 에 없는 항목을 DB 에서 자동 삭제한다. JSON 에서 항목을 빼면 DB 에서도 사라진다.

### 5.5.5 명령어 의사결정 — 언제 어떤 make 타겟을 쓰나

> "지금 무엇을 바꿨는지" 만 보고 어디까지 실행할지 결정. 가장 자주 헷갈리는 부분.
...
flowchart TD
    ...
    Q --> S6[새 인물 아바타 추가<br/>assets/avatars/*.png]
    S6 --> C6[make thumbnails<br/>+ upload-character-avatars]

    Q --> S7[DB 스키마 자체 변경<br/>db_init.sql 수정]
    S7 --> C7[⚠️ make db-init<br/>모든 데이터 사라짐 — 풀 리셋 시퀀스 사용]

    Q --> S8{풀 리셋<br/>전부 처음부터}
    S8 --> C8[make seed-all<br/>&& make db-init<br/>&& make apply-seeds<br/>&& make upload-character-avatars]
...
sequenceDiagram
    actor U as 운영자
    participant Local as 로컬 디스크<br/>(assets/, tools/)
    participant SQL as supabase/<br/>(생성된 SQL)
    ...
    Local->>Storage: assets/avatars/*.png → characters/<code>.png

    Note over U: 완료 — 앱 실행 시 모든 콘텐츠 정상
```

**한 줄 명령**:
```bash
make seed-all && make db-init ENV=dev && make apply-seeds ENV=dev && make upload-character-avatars ENV=dev
```
...
- quiz JSON/export도 current active events 기준이라, 삭제 후 `story_index`가 당겨져도
  옛 `era_xxx_nNNN.json`이 다른 이야기의 퀴즈로 재사용되지 않는다.
