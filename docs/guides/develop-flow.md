...
| 구분 | 위치 | 의미 |
|------|------|------|
| 초기 bootstrap 원본 | `assets/initial_seed_events/*.json`, `assets/landmarks/landmarks.json`, `assets/bible/*.txt`, `assets/story_images/`, `assets/avatars/` | 새 Supabase 프로젝트를 처음 세팅할 때 쓰는 보존 seed이며 `make seed-*`의 기본 입력이다. |
| 운영 active snapshot | `assets/events/*.json` | DB의 active published events를 export한 release snapshot이다. 운영 중 승인/삭제/재정렬 후 앱 번들 썸네일과 `story-image-sources` 원본 archive를 맞출 때 기본으로 쓴다. |
...
주의할 점은 `make seed-all`이 기본적으로 `assets/initial_seed_events/*.json`만 믿는다는 것이다. 초기 원본 seed를 보존하고, 운영 DB의 active 상태를 기준으로 seed 산출물을 만들 필요가 있는 예외 상황에서만 먼저 `make export-stories-json ENV=real`로 DB snapshot을 받은 뒤 `make seed-all STORIES_DIR=assets/events`처럼 명시한다.

초기 bootstrap 직후 `upload-story-image-sources`까지 실행하려면 `make apply-seeds ENV=real` 다음에 `make export-stories-json ENV=real`을 한 번 실행해 `assets/events`를 방금 만든 DB active snapshot으로 맞춘다. source archive sync는 항상 `assets/events` 기준으로 움직이기 때문이다.
...
| `build-character-meta` | `$(STORIES_DIR)/*.json`의 `characters`, `scene_characters` (기본 `assets/initial_seed_events`) | `tools/seed/character_meta.json` | 인물 seed, events 인물 코드 검증, 아바타 생성 후보에 쓰인다. |
| `seed-stories` | `$(STORIES_DIR)/*.json`, `tools/seed/character_meta.json` | `supabase/events/events_seed.sql`, `supabase/events/events_seed_part_*.sql`, `supabase/events/events_report.json`, `supabase/events/events_normalized.json` | `make apply-seeds`가 `events` 테이블에 넣는다. |
| `seed-characters` | `tools/seed/character_meta.json`, `$(STORIES_DIR)/*.json` | `supabase/events/characters_seed.sql` | `make apply-seeds`가 `characters` 테이블에 넣는다. |
| `seed-quizzes` | `$(STORIES_DIR)/*.json` 안의 `quiz_questions` | `supabase/quizzes/quizzes_seed.sql`, `supabase/quizzes/quizzes_report.json` | `make apply-seeds`가 `quiz_questions` 테이블에 넣는다. |
...
$(STORIES_DIR)/*.json + tools/seed/character_meta.json
  -> supabase/events/events_seed_part_*.sql
  -> DB events

$(STORIES_DIR)/*.json 안의 quiz_questions
...
`make seed-all`은 썸네일을 만들지 않는다. 앱에 포함될 이미지까지 운영 active snapshot과 맞추려면 다음으로 `make thumbnails`를 실행한다. `make thumbnails`는 seed용 `STORIES_DIR`가 아니라 `THUMBNAIL_STORIES_DIR`를 읽으며, 기본값은 `assets/events`다. 초기 bootstrap 이미지 세트를 만들 때만 `THUMBNAIL_STORIES_DIR=assets/initial_seed_events`를 명시한다.

| 입력 | 출력 | 동작 |
|------|------|------|
| `$(THUMBNAIL_STORIES_DIR)/*.json` (기본 `assets/events`) | `assets/story_images_thumbs/index.json` | 각 이야기 title, era, story_index와 짧은 썸네일 디렉토리의 매핑을 만든다. |
...
story thumbnail은 매번 clean rebuild다. 먼저 current `$(THUMBNAIL_STORIES_DIR)/*.json`에 필요한 원본 PNG가 모두 있는지 확인하고, 통과하면 `assets/story_images_thumbs/`의 story 하위 디렉토리를 지운 뒤 현재 `era + story_index` 기준으로 다시 만든다. 그래서 중간 삽입이나 삭제로 뒤쪽 story_index가 바뀌어도 예전 썸네일 디렉토리가 앱 번들에 남아 잘못 매칭되는 일을 막는다.
...
| 5 | `supabase/quizzes/quizzes_seed.sql` | `quiz_questions` | 각 event의 기존 quiz row를 지운 뒤 seed 생성에 사용한 `$(STORIES_DIR)/*.json` 안의 `quiz_questions`를 다시 insert한다. |
