...
assets/                                 # 입력/출력 에셋
├── events/*.json                       # 이야기 + landmark_code + quiz_questions 통합 소스
...
- **입력**: `assets/events/*.json` + `tools/seed/character_meta.json`. 각 event row가 `landmark_code`를 직접 가진다.
...
- **장면 캡션**: `scene_captions`는 `assets/events/*.json`에서 직접 수정 가능하다. `tools/seed/generate_scene_captions.py`는 기존 `story_scenes`/`summary`/`bible_ref` 맥락에서 프롬프트 지시문을 제거한 초안을 다시 만들 때만 사용한다.
- **시간 순 구간**: `unit_code`/`unit_title`/`unit_order`는 `assets/events/*.json`이 원본이다. 구약 시대는 원역사 2개, 족장 3개, 출애굽 3개, 사사 3개, 왕정 3개, 분열왕국 5개, 포로/귀환 3개 구간으로 나눠 `TimelineUnitPickPanel`의 가로 카드 선택에 사용한다.
...
- **입력**: `tools/seed/character_meta.json` + `assets/events/*.json` (대표 스토리 선택용)
...
- **era_codes**: `assets/events` 의 실제 등장 사건을 기준으로 계산한다. 인물이 여러 시대에 등장하면 여러 `era_code` 를 가진다. 등장 사건이 없는 메타 항목만 아바타 스타일용 era 를 fallback 으로 사용한다.
...
- **입력**: `assets/events/*.json` 안의 `quiz_questions` 배열. 같은 event row의 `bible_ref`로 해설 절 범위를 검증한다.
- **출력**:
...
- **입력**: `assets/events/*.json` (story_scenes 필드)
...
- **입력**: `assets/avatars/`, `assets/story_images/`, `assets/events/*.json`
...
make seed-stories            # build_events_seed_sql.py (→ person-meta 의존)
make seed-characters            # build_characters_seed_sql.py (→ person-meta 의존)
make seed-quizzes            # build_quizzes_seed_sql.py (→ assets/events quiz_questions)
...
```
assets/
├── events/                   # 통합 소스 JSON (이야기 + landmark_code + quiz_questions)
│   ├── era_primeval.json
│   ├── era_patriarch.json
│   ├── era_exodus.json
│   ├── era_judges.json
...
### 신규 이야기 1건 추가된 경우
⚠️ **반드시 먼저**: 로컬 `assets/events/`가 DB와 동기화된 상태여야 한다. 로컬이 비었거나 오래됐다면 `make export-stories-json` (운영 기준은 `ENV=real` 또는 `ENV=prod`) 으로 DB의 active published events(`deleted_at IS NULL`)를 JSON으로 역추출해 복원한다. 이 사전 조건을 빼먹고 부분 상태에서 빌드하면 기존 인물 description이 손상된다.
