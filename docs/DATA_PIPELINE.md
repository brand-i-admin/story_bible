...
- **`assets/initial_seed_events/*.json`** — 초기 DB bootstrap/레거시 원본 보존용 시대별 이야기 JSON. 사람이 의도하고 AI(LLM)로 초안을 뽑아 정리한 원본 seed이며, `make seed-*`/`make generate-story-images`의 기본 입력이다.
- **`assets/events/*.json`** — DB의 active published events를 역추출한 release snapshot. 운영 중 승인/삭제/재정렬이 끝난 최신 상태를 로컬에서 관리하며, `make release-sync-stories`, 썸네일 생성, `story-image-sources` 원본 archive sync의 기준이다. 운영 active snapshot으로 seed SQL을 만들어야 할 때만 `STORIES_DIR=assets/events`를 명시한다. 신규 이야기는 `assets/story_drafts/*.json`에서 proposal로 올리고 승인 후 `make release-sync-stories` 단계에서 이 snapshot으로 편입한다.
...
assets/                                 # 입력/출력 에셋
├── initial_seed_events/*.json                  # 초기 bootstrap/레거시 원본 seed JSON
├── events/*.json                       # DB active snapshot + release 통합 소스
└── landmarks/landmarks.json            # 지도 region/landmark 카탈로그
...
```
                    ┌─────────────────────────────────────────────┐
                    │          $(STORIES_DIR)/*.json          │
                    │   (기본 assets/initial_seed_events, active seed 시 assets/events) │
                    └──┬──────────────────────────┬───────────────┘
                       │                          │
                       ▼                          ▼
...
- **입력**: Makefile 기준 기본 `assets/initial_seed_events/*.json` (`STORIES_DIR`로 변경 가능, 운영 active snapshot으로 만들 때 `assets/events`). ⚠️ **로컬에 있는 파일만** 스캔 — 부분 스캔 주의
...
#### `build_events_seed_sql.py` — events SQL
- **의존**: `tools/seed/character_meta.json` (인물 화이트리스트). 입력 JSON 각 항목에 `story_index` 키가 있어야 함 (없으면 `ValueError` 즉시 raise).
- **입력**: Makefile 기준 기본 `assets/initial_seed_events/*.json` + `tools/seed/character_meta.json` (`STORIES_DIR`로 변경 가능). 각 event row가 `landmark_code`를 직접 가진다.
- **스키마 원본**: 현재처럼 실사용자 없는 초기화 단계에서는 `db_init.sql`이 기준이다.
  별도 schema patch 파일은 만들지 않는다. `background_context`, `scene_captions`,
  `unit_code`/`unit_title`/`unit_order`, `events_ordered` view는 `db_init.sql`이
  기준이고, 기존 운영 DB를 보존하며 바꿔야 할 때만 `supabase/patches/*.sql`에
  별도 idempotent patch를 작성한다.
- **출력**:
  - `supabase/events/events_seed.sql` — `events` 한 테이블 INSERT (배열 + JSONB 컬럼 포함)
...
- **split 파일 주의**: `events_seed_part_01.sql` 만 stale-delete 를 포함, part_02 는 INSERT 만.
...
- **입력**: `tools/seed/character_meta.json` + Makefile 기준 기본 `assets/initial_seed_events/*.json` (대표 스토리 선택용, `STORIES_DIR`로 변경 가능)
- **출력**: `supabase/events/characters_seed.sql` — `characters` UPSERT. `tagline`은 인물 선택 카드에 쓰는 짧은 정체성 문구로, 왕은 나라/순서/대표 행적을, 사사와 선지자는 함께 활동한 인물이나 대표 사역을 최대한 담는다(예: `북이스라엘 7대 왕 바알 숭배`, `바락과 승리한 여사사`, `히스기야를 도운 예루살렘 선지자`). `is_active_default=true`인 행은 seed 적용 시 기존 DB 행도 `is_active=true`로 승격하고, 일반 `false` 기본값은 기존 런타임 설정을 끄지 않는다. 단, `god`, `gabriel`, `elizabeth`처럼 명시 비활성 예외인 코드는 seed 적용 시 `false`로 강제한다.
- **era_codes**: 선택한 `STORIES_DIR`의 실제 등장 사건을 기준으로 계산한다. 인물이 여러 시대에 등장하면 여러 `era_code` 를 가진다. 등장 사건이 없는 메타 항목만 아바타 스타일용 era 를 fallback 으로 사용한다.
...
- **입력**: Makefile 기준 기본 `assets/initial_seed_events/*.json` 안의 `quiz_questions` 배열 (`STORIES_DIR`로 변경 가능). 같은 event row의 `bible_ref`로 해설 절 범위를 검증한다.
- **출력**:
...
- **보기 구성**: JSON 원본은 선택지 3개와 `answer_index` 0~2만 가진다. SQL 생성 시 `choice_d='헷갈렸어요'`를 자동 추가해 앱에서는 항상 4번 보기로 노출한다. DB export로 이미 섞인 보기 순서가 들어와도 `answer_index`를 보존한 채 deterministic shuffle을 적용한다. 보기는 본문 표현을 그대로 찾게 하지 말고, 사용자가 사실을 이해했는지 고를 수 있는 일상적인 한국어 문장으로 쓴다.
...
- **출력**: `supabase/events/landmarks_seed.sql` — `landmarks` UPSERT
...
- **입력**: Makefile 기준 기본 `assets/initial_seed_events/*.json` (story_scenes 필드, `STORIES_DIR`로 변경 가능)
...
- **옵션**: `--no-prune-orphans` (현재 stories JSON에 없는 원본 이미지 디렉토리 정리 스킵)
...
- **입력**: `assets/avatars/`, `assets/story_images/`, `$(THUMBNAIL_STORIES_DIR)/*.json` (기본 `assets/events`)
- **출력**: `assets/avatars_thumbs/`, `assets/story_images_thumbs/`
- **방식**: 로컬 이미지 리사이즈 (Vertex AI 불필요)
- **옵션**: `--no-prune-orphans` (story clean rebuild와 avatar orphan 정리 스킵)
- **원본 필수 조건**: `assets/story_images/`는 `.gitignore` 대상인 로컬 원본 staging
  디렉토리다. `make thumbnails`는 current `assets/events/*.json`의 모든 active
...
- **story thumb clean rebuild**: current `assets/events/*.json`에 등장하는
  title 폴더만 썸네일 생성 대상으로 삼는다. 누락 원본 검사를 먼저 통과한 뒤
  `assets/story_images_thumbs/`의 story 하위 디렉토리를 모두 지우고, 현재
  `era + story_index` 기준 짧은 디렉토리를 처음부터 다시 만든다. `index.json` 같은
  파일은 보존/갱신하고, avatar thumb은 기존처럼 missing/changed만 갱신한다.
- **중간 삽입 안전성**: 짧은 디렉토리명은 `primeval_003`처럼 위치 기반이다. 새
  이야기가 중간에 들어오면 뒤쪽 이야기의 디렉토리명이 바뀌거나 같은 디렉토리명이
  다른 이야기를 가리킬 수 있다. 그래서 story thumb은 mtime skip에 기대지 않고 매번
  clean rebuild한다. 삭제 승인 후 로컬 `assets/story_images/<삭제된 제목>/` 원본 폴더가
  실수로 남아 있어도 current JSON에 없으면 앱 번들 썸네일로 다시 살아나지 않는다.
...
- **입력**: `assets/events/*.json`, `assets/story_images/<title>/scene_*.png`,
...
- **Make target**:
  ...
  - `make upload-story-image-sources ENV=real` — 로컬 current story 원본 중
    missing/changed PNG를 업로드/upsert하고, old manifest에만 있는 stale object를 삭제한 뒤
    active manifest 갱신
  - `*-dry` 변형은 실행 계획만 확인
...
- **push 세부 기준**:
  - old manifest에 없고 active local set에 있는 object: 새 업로드
  - old manifest와 같은 `object_path`지만 `size` 또는 `sha256`이 다른 object: Supabase
    Storage `x-upsert: true`로 덮어쓰기
  - old manifest에는 있지만 active local set에는 없는 object: stale 삭제
  - old manifest와 active local set이 같은 `size`/`sha256`: skip
- **삭제 범위**: stale 삭제는 bucket 전체 listing이 아니라
  `_manifests/story_images_manifest.json`의 이전 entries와 현재 active entries 비교로만
  결정한다. manifest에 기록되지 않은 수동 파일은 이 도구가 삭제 대상으로 삼지 않는다.
...
- **입력**: `assets/landmarks/landmarks.json` + `assets/events/*.json`
- **출력**: stdout 에 `OK: N events, all inside their region polygon.` (성공) 또는 stderr 에 위반 목록 + exit code 1 (실패)
- **목적**: 모든 사건의 `landmark_code`가 가리키는 landmark 좌표가 소속 region polygon 안에 있는지 ray-casting 으로 검증. 새 이야기 추가 시 핀이 색칠된 영역 밖에 표시되는 시각 모순을 차단.
- **옵션**: `--landmarks <path>`, `--stories <path>` (디렉토리이면 *.json 글롭). 기본값은 프로젝트 표준 경로.
- **자동 실행**: `.pre-commit-config.yaml` 의 pre-push 훅 — `landmarks.json` 또는 `assets/events/*.json` 변경 시 push 전에 검증.
...
DB-first release 동기화는 `make export-stories-json` 하나를 canonical export로 사용한다.

- `make export-stories-json`: active published `events`(`status='published' AND deleted_at IS NULL`)를 `assets/events/*.json`으로 역추출한다. 각 row에는 `landmark_code`와 연결된 `quiz_questions`가 함께 들어간다. soft-deleted row 는 DB에 남아도 canonical JSON 에 다시 섞지 않는다.
- `make release-sync-stories`: 통합 event export, 승인 proposal 이미지 동기화, 원본 archive 보강, 썸네일 생성, pubspec 갱신을 순서대로 실행한다. 여러 approved proposal과 delete approval이 한 번에 쌓여 있어도 전체 DB 상태를 로컬 release 번들로 당겨온다.
- `make seed-stories`/`make seed-characters`/`make seed-quizzes`/`make generate-story-images`: 기본 입력은 `assets/initial_seed_events`다. 운영 active snapshot으로 seed 산출물이 필요한 예외 상황에서만 `STORIES_DIR=assets/events`를 명시한다.
- `make thumbnails`: 기본 입력은 active snapshot인 `assets/events`다. 초기 seed 원본 기준 썸네일을 만들 때만 `THUMBNAIL_STORIES_DIR=assets/initial_seed_events`를 명시한다.
...
1. `export-stories-json`: `events` 직접 조회 시 `status='published' AND deleted_at IS NULL`을 함께 건다. era별 `assets/events/*.json`을 덮어쓰며, 각 event row에는 `landmark_code`, `place_name/lat/lng`, `quiz_questions`가 포함된다. 현재 active export에 없는 stale `era_*.json`/`unknown_era.json` 파일은 제거한다.
2. `sync-approved-proposal-assets --skip-post-processing` Phase A: 아직 `synced_to_local_at`이 없는 approved NEW proposal의 장면 이미지를 `proposal-scenes`에서 `assets/story_images/<canonical title>/scene_N.png`로 내려받고, 신규 캐릭터 PNG는 `assets/avatars/<code>.png`와 `characters/<code>.png` Storage로 복사한다. 성공한 proposal만 sync marker를 set하므로 중간 실패는 다음 실행에서 재시도된다.
3. `sync-approved-proposal-assets --skip-post-processing` Phase B: DB active event title 집합과 로컬 `assets/story_images/`를 비교해 active set에 없는 이야기 폴더를 삭제한다. 과거 sync 버그로 공백이 `_`로 바뀐 폴더는 canonical 제목 폴더로 먼저 옮긴다. soft-deleted events의 `scene_image_paths` Storage fallback은 best-effort로 삭제한다. 캐릭터는 DB에 존재하지 않는 code의 로컬 avatar/thumb만 삭제한다.
4. `ensure-story-image-sources`: `story-image-sources` private bucket manifest와 로컬 원본을 비교해, proposal sync로 채워지지 않은 missing/changed 원본 PNG를 다운로드한다.
5. `make thumbnails`: `assets/events/*.json`에 등장하는 current story title만 썸네일 대상으로 삼는다. 누락 원본 검사를 통과한 뒤 `assets/story_images_thumbs/`의 story 하위 디렉토리를 clean rebuild하고, `index.json`과 짧은 디렉토리를 현재 `era + story_index` 기준으로 다시 만든다. 로컬에 삭제된 이야기 원본 폴더가 남아 있어도 current JSON에 없으면 번들 썸네일로 살아나지 않는다.
6. `make update-pubspec-assets`: 현재 `story_images_thumbs/index.json`과 실제 짧은 디렉토리 목록을 `pubspec.yaml`의 asset 엔트리에 반영한다.
7. `upload-story-image-sources`: proposal sync로 새로 생긴 원본을 포함해 local source cache의 신규/변경 PNG를 private source archive에 upsert하고, 이전 manifest에만 남은 object를 삭제한 뒤 active manifest를 갱신한다.
...
4. `upload-story-image-sources`가 신규/변경 원본만 원본 저장소에 upsert하고, old manifest에는 있지만 active set에는 없는 stale 원본 object를 삭제한 뒤 manifest를 갱신한다.
...
4. 운영자가 `make release-sync-stories ENV=real`을 실행한다. 로컬 `assets/events`는 active 10건만 담고, 삭제된 두 title은 통합 JSON, `assets/story_images_thumbs/index.json`, `pubspec.yaml`에서 빠진다. `assets/story_images/<삭제된 title>/` 원본 폴더도 active-title diff에서 삭제된다. 새 A/B 장면 이미지는 proposal Storage에서 로컬로 내려오고 짧은 썸네일 디렉토리가 새 `story_index` 기준으로 만들어진다.
5. 이후 `make seed-stories-characters`/`make seed-quizzes`를 다시 돌려도 events JSON 안의 `landmark_code`와 `quiz_questions`가 active DB 기준이라 삭제된 story_index gap이나 stale quiz가 seed에 섞이지 않는다.
...
make seed-stories            # build_events_seed_sql.py (기본 assets/initial_seed_events, → person-meta 의존)
make seed-characters            # build_characters_seed_sql.py (기본 assets/initial_seed_events, → person-meta 의존)
make seed-quizzes            # build_quizzes_seed_sql.py (기본 assets/initial_seed_events quiz_questions)
...
make export-stories-json     # DB events + quiz_questions + landmark_code → assets/events
...
운영 active snapshot을 기준으로 seed SQL과 이미지를 만들 때는 입력 디렉토리를 명시한다.

```makefile
make seed-all STORIES_DIR=assets/events
make generate-all STORIES_DIR=assets/events
```

> **`story_index`**: `assets/events/*.json` 의 각 이야기에 직접 박힌 정수 (era 내 1..N).
...
```
assets/
├── initial_seed_events/              # 초기 bootstrap/레거시 원본 seed JSON
├── events/                   # DB active snapshot + release 통합 JSON
│   ├── era_primeval.json
│   ├── era_patriarch.json
│   ├── era_exodus.json
│   ├── era_judges.json
...
├── story_images/             # 원본 장면 이미지 (제목 폴더 기준, events JSON의 story_scenes와 동기화)
...
```json
{
  ...
  "era": "era_primeval",
  "characters": ["god"],
  "landmark_code": "lm_primeval_mesopotamia",
  ...
  "quiz_questions": [
    {
      "type": "fact",
      "display_order": 0,
      "question": "하나님은 무엇으로 세상을 창조하셨습니까?",
      "choices": ["말씀", "칼", "성벽"],
      "answer_index": 0,
      "explanation": "창 1:3 — '하나님이 가라사대 빛이 있으라 하시매 빛이 있었고'"
    }
  ]
}
```
...
- `landmark_code`: `assets/landmarks/landmarks.json`의 `regions` 또는 `landmarks` code. events seed는 이 값으로 `events.landmark_id`를 조인한다.
- `quiz_questions`: `quiz_questions` seed의 원본. 이전처럼 별도 퀴즈 파일과 DB 이벤트 스냅샷 파일을 나누어 관리하지 않는다.
...
2. `make seed-stories-characters` → 초기 원본 기준 `characters_seed.sql` + `events_seed_part_*.sql` 생성
...
8. `make thumbnails THUMBNAIL_STORIES_DIR=assets/initial_seed_events` (초기 원본 기준 앱 번들용 썸네일 생성)
...
```bash
make seed-all
make thumbnails THUMBNAIL_STORIES_DIR=assets/initial_seed_events
...
### 신규 이야기 1건 추가된 경우
표준 운영 경로는 `assets/story_drafts/*.json` 작성 → `make apply-draft ENV=real` → 관리자 승인 → `make release-sync-stories ENV=real`이다. 이 경로에서는 DB가 먼저 active row와 quiz를 확정하고, release sync가 그 결과를 `assets/events`, `assets/story_images`, `assets/story_images_thumbs`, `pubspec.yaml`, `story-image-sources` manifest로 당겨온다.

운영 active snapshot을 seed SQL로 다시 만들어 검수해야 하는 예외 상황에서만 `make seed-stories-characters STORIES_DIR=assets/events`나 `make seed-quizzes STORIES_DIR=assets/events`처럼 명시한다. 기본 `make seed-*`는 초기 bootstrap 보존 원본인 `assets/initial_seed_events`를 사용한다.
