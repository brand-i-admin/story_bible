...
글자 = 글자 크기 변경 — 프로필과 알림 사이에서 `FontScaleBottomSheet`를 연다.
```
...
갈색 버튼, 다음 계열은 우측 초록 버튼을 쓰며, 버튼 뒤에는 별도 사각 음영을 두지 않는다.
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | 프로필 탭 (아바타/이름/수정·설정 헤더는 첫 컨테이너 위에 분리, 아바타·이름과 기도 탭의 내 기도 텍스트는 프로필 수정 진입점, 기록, 기도, 저장한 이야기, 저장한 말씀, 진행도). 좌측 `기록/기도/저장/말씀` 탭은 우측 진행도와 다이어리 탭과 같은 초록 segmented 버튼 톤을 사용하며, 컨테이너 가로폭을 꽉 채우고 같은 연한 배경색을 쓴다. 저장/말씀 탭을 누르면 미리보기 데이터를 다시 불러와 방금 저장한 이야기와 말씀이 바로 보이게 한다. 설정 시트에는 개인정보 보호, 지도 설명, 로그아웃, 계정 삭제를 둔다. 계정 삭제는 별도 확인 다이얼로그에서 표시된 계정 확인 아이디를 직접 입력해야 최종 버튼이 활성화된다. 진행도 탭은 `나의 다이어리 → 인물과 걷기 → 장소로 시작` 순서이며 인물은 첫 등장 이야기 순서로 나열 |
...
1. 홈 상단 **미션** 버튼은 아이콘 없이 텍스트만 표시하고 상단 유틸리티 아이콘 버튼과 같은 높이를 쓴다. 오늘 매일 미션 사건이 이미 완료된 이야기이거나 오늘 KST 감정을 새겼으면 하단 스태퍼의 초록색 배경과 흰 글자로 고정하고, 아직 진행하지 않은 사건이면 하단 스태퍼의 노란색 배경과 흰 글자 반짝임으로 클릭을 유도한다. 상태 기준은 주간 미션이 아니라 매일 미션이다.
...
13. 프로필 좌측 기록/기도/저장/말씀 탭은 우측 `나의 다이어리/인물과 걷기` 및 `오늘의 신앙 기록/오늘의 내 감정` 탭과 같은 초록 segmented 버튼 UI를 쓰고, 탭 바는 좌측 패널 가로폭을 채우며 동일한 연한 베이지 배경을 사용한다. 컨테이너는 콘텐츠 높이에 맞춰 가변적으로 표현한다. 기록 탭 상단의 이야기/통독 전체 진행률 카드는 퍼센트 대신 제목보다 작은 완료 수/전체 수를 프로그레스 바 중앙에 겹쳐 표시하고, 위쪽 row는 아이콘과 제목만 사용해 `이야기 진행률`, `통독 진행률` 라벨이 한 줄로 보이게 한다. 이야기는 감정 새기기까지 끝난 이야기 수, 통독은 완료한 장 수를 기준으로 한다. 통독 진행률 팝업에서 장 번호를 누르면 선택한 권/장으로 성경 리더를 열어 마지막 절 아래의 통독 읽음 처리까지 이어지게 한다. 기록 탭은 정답/오답/헷갈려요 통계를 높이를 확보한 버튼형 카드 3개로 보여 주고, 아이콘+라벨과 문항 수/이야기 수는 각 카드 중앙에 배치한다. 감정 통계 칩 8개는 `나의 다이어리` 달력 연월 헤더 위에 개수순 4열로 나열하고, 칩을 누르면 해당 감정을 새긴 이야기 목록을 같은 리뷰 다이얼로그로 표시한다.
