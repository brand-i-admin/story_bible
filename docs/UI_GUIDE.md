...
| ProfileTabPage | `widgets/profile_tab_page.dart` | 프로필 탭 (아바타/이름/수정·설정 헤더는 첫 컨테이너 위에 분리, 아바타·이름과 기도 탭의 내 기도 텍스트는 프로필 수정 진입점, 기록, 기도, 저장한 이야기, 저장한 말씀, 진행도). 저장/말씀 탭을 누르면 미리보기 데이터를 다시 불러와 방금 저장한 이야기와 말씀이 바로 보이게 한다. 설정 시트에는 개인정보 보호, 지도 설명, 로그아웃, 계정 삭제를 둔다. 계정 삭제는 별도 확인 다이얼로그에서 표시된 계정 확인 아이디를 직접 입력해야 최종 버튼이 활성화된다. 진행도 탭은 `나의 다이어리 → 인물과 걷기 → 장소로 시작` 순서이며 인물은 첫 등장 이야기 순서로 나열 |
...
5. 결과는 선택 구절 카드 아래 `StoryEventThumbCard` 기반 카드 그리드로 표시
6. 카드 탭 → 홈의 기존 이야기 상세/본문 읽기/퀴즈/감정 새김 흐름으로 이동

### 5.2.2 매일/주간 탐험 카드

1. 매일 탐험은 상단 소개 영역에서 네비게이션 아이콘 + **오늘의 탐험** 라벨을 먼저 보여 주고, 그 아래 시대 이모지+텍스트, 인물 avatar+텍스트, 핀 이모지+장소 텍스트로 오늘의 여정을 말한다. 이 소개 문장 안에는 별도 rounded chip 을 쓰지 않는다
2. 매일 탐험은 소개 아래 본 탐험 블록을 하나의 rounded rectangle 로 묶고, 맨 위에 나침반 이모지 + **오늘의 사건** 라벨을 둔다. 그 아래 "「사건명」 사건" 메인 버튼은 이모지나 선택 라벨 없이 가운데 정렬된 굵은 제목처럼 길게 배치하고, **인물 루트 / 장소 사건**은 `함께 보기` 보조 버튼 row 로 분리한다. 오늘 사건은 항상 핵심 CTA 이고, 인물 루트와 장소 사건은 오늘 사건을 입체적으로 보는 보조 탐색이다. 장소 사건은 1개만 있어도 눌러 볼 수 있으며 사건 수를 그대로 표시한다
3. 매일 탐험 본 탐험 블록은 버튼 row 아래 지도를 충분히 확보하고, 필요하면 구분선과 사건 카드 row 는 세로 스크롤 아래에서 이어 보여 준다. 인물 루트 지도에서는 인물 코드 legend 를 숨긴다. 같은 사건에 오늘 KST 감정을 새겼으면 축복 문구를, 이전 날짜 감정만 있으면 재탐험 문구를 구분선과 사건 카드 row 사이에 표시한다. 감정 기록이 없거나 인물/장소 보조 보기에서는 추가 문구를 숨긴다
4. 주간 탐험은 인물/지역 헤더와 지도, 사건 카드 row 만 보여 주며 매일 탐험 전용 재탐험/축복 문구를 쓰지 않는다
...
13. 프로필 좌측 기록/기도/저장/말씀 컨테이너는 콘텐츠 높이에 맞춰 가변적으로 표현한다. 기록 탭은 정답/오답/헷갈려요 통계를 높이를 확보한 버튼형 카드 3개로 보여 주고, 아이콘+라벨과 문항 수/이야기 수는 각 카드 중앙에 배치한다. 감정 통계 칩 8개는 `나의 다이어리` 달력 연월 헤더 위에 개수순 4열로 나열하고, 칩을 누르면 해당 감정을 새긴 이야기 목록을 같은 리뷰 다이얼로그로 표시한다.
14. 프로필 말씀 탭은 전체 저장 말씀 화면과 같은 `SavedVerseRow`를 재사용하고, 미리보기는 최신순 최대 3개와 `...`를 보여 준 뒤 하단 전체 보기로 전체 목록에 진입한다. 저장한 성경 구절 전체보기와 동행 일지 리스트는 상단에 아이콘형 이전 버튼과 화면 제목을 같은 row로 배치하고, 본문 카드는 버튼 폭에 밀리지 않도록 좌우 전체 폭을 사용한다
...
2. 인라인 로그인 카드는 카카오/Google/Apple 소셜 로그인만 제공한다. 모바일 OAuth는 앱 바깥 기본 브라우저가 아니라 인앱 브라우저/Safari View Controller 계열 launch mode를 사용한다.
3. 적용 대상: 사건 상세 **퀴즈 시작**, 성경 구절 별표 저장, 성경 화면 우측 상단 저장 말씀 버튼, 사건 읽기 완료 저장, 이야기 저장, 감정 새김.
4. 성경 읽기 자체와 매일/주간 탐험 목록은 비로그인도 열 수 있지만, 마지막 **읽기 완료**나 퀴즈 결과 저장처럼 사용자별 진행도 저장을 시도하면 로그인 유도로 돌아간다.
