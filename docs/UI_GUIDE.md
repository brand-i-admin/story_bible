...
| **2a. White Wash** | `AppColors.regionParchmentWash` (#FFF7E8) alpha **0.45**, `MaskFilter.blur(solid, 2.5)` | **베이지 양피지 베이스 중성화** — 색 fill 위로 베이스가 비쳐 갈색·짙은녹으로 흐려지는 문제 차단. 지도 질감은 wash 너머로 은은히 비침 |
...
MapLibre polygon layer 는 단색 fill 중심이라 multi-pass wash 효과를 Flutter
`CustomPainter` 레이어로 분리한다. 운영 3D 지도의 클릭 hit-test 는 WebView 내부
MapLibre GeoJSON fill layer 가 담당하고, Flutter 레이어는 시각 효과에 집중한다.
