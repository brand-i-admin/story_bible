...
16. 프로필 "나의 다이어리" 탭은 기존 지도형 장식을 쓰지 않고 한국 시간 기준 달력으로 감정 기록을 보여 준다. 접힌 상태에서는 지난주와 이번주 두 줄만 보여 주고, 펼치면 월 전체와 이전/다음 월 이동을 제공한다. 날짜 셀은 얇은 구분선만 사용하고 오늘은 날짜 숫자 동그라미, 선택 날짜는 은은한 배경으로 구분한다. 하루 감정 표시는 한 줄 2개, 최대 2줄이며 3개까지는 아이콘을 노출하고 4개 이상은 두 번째 줄에 `+x`로 남은 개수를 요약한다. 주차 높이는 해당 주에서 가장 많이 기록된 날짜의 표시 줄 수에 따라 가변적으로 잡아 빈 공간을 줄인다.
