...
보여 주며, 시대 선택과 보기 방식 선택은 `①`, `②` 원형 숫자 배지로 나눠 보여준다.
첫 화면 제목과 단계 문구는 좁은 폰에서도 한 줄을 유지하도록 축소 렌더링한다.
...
| EventDetailPage | `widgets/event_detail_page.dart` | 사건 상세 페이지 (제목 아래 연대/장소 메타 + 하나님 우선·가나다 순으로 겹치지 않게 나열되는 등장인물 아바타 + 탭하면 전체 설명으로 뒤집히는 장면 이미지 + 문항별 해설 퀴즈) |
| WeeklyTabPage | `widgets/weekly_tab_page.dart` | 금주의 인물 탭 |
| ProfileTabPage | `widgets/profile_tab_page.dart` | 프로필 탭 (아바타/이름/수정·설정 헤더는 첫 컨테이너 위에 분리, 아바타·이름과 기도 탭의 내 기도 텍스트는 프로필 수정 진입점, 기록, 기도, 저장한 이야기, 저장한 말씀, 진행도). 설정 시트에는 개인정보 보호, 글자 크기 변경, 지도 설명, 로그아웃을 둔다. 진행도 탭은 `나의 다이어리 → 인물과 걷기 → 장소로 시작` 순서이며 인물은 첫 등장 이야기 순서로 나열 |
