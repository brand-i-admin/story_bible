...
16. 프로필 "나의 다이어리" 탭은 기존 지도형 장식을 쓰지 않고 한국 시간 기준 달력으로 감정 기록을 보여 준다. 접힌 상태에서는 지난주와 이번주 두 줄만 보여 주고, 펼치면 월 전체와 이전/다음 월 이동을 제공한다. 날짜 셀은 얇은 구분선만 사용하고 오늘은 날짜 숫자 동그라미, 선택 날짜는 은은한 배경으로 구분한다. 하루 감정 표시는 한 줄 2개, 최대 2줄이며 3개까지는 아이콘을 노출하고 4개 이상은 두 번째 줄에 `+x`로 남은 개수를 요약한다. 주차 높이는 해당 주에서 가장 많이 기록된 날짜의 표시 줄 수에 따라 가변적으로 잡아 빈 공간을 줄인다. 달력 아래 감정 카테고리 칩은 누르면 해당 감정 이야기 팝업을 열고, 감정 row 탭은 홈 지도/지역 복원 준비를 기다리지 않고 사건 상세 페이지로 바로 진입하며, 짧은 전환 로딩만 패널 입력을 막는다. 다만 감정을 다시 저장하는 순간에는 홈 지도 루트로 돌아가 도장 애니메이션을 보여 준 뒤 상세로 복귀한다.
