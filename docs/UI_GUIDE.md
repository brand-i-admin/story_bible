...
| BibleReaderPage | `widgets/bible_reader_page.dart` | 성경 리더 페이지 (구절 탭 선택 + 하단 액션바 + 이야기 본문 읽기 모드). 구절 본문을 탭하면 해당 절에 밑줄과 좌측 강조가 붙고, 하단 액션바에 파랑/노랑 하이라이트, 저장, 복사 버튼을 보여 준다. 저장은 200자 묵상 코멘트 팝업을 띄우고 빈 코멘트도 저장하며, 하이라이트는 코멘트 없이 즉시 저장한다. 복사는 `권 장:절 본문` 형식을 클립보드에 넣는다. 저장 취소는 저장 말씀 화면의 삭제 버튼에서 코멘트 유무에 따라 확인 팝업 또는 즉시 취소 스낵바를 사용한다. 일반 성경 진입은 책/장 탐색을 유지하고, 장 단위 **통독 읽음 처리**는 하단 고정 이전/다음 장 바가 아니라 마지막 절 아래에 둔다. 사건 상세에서 진입하면 해당 `bible_refs` 범위의 절만 표시한다. 여러 본문은 **다음**으로 순차 이동하고 마지막 본문에서 **읽기 완료**를 눌러야 읽음 처리된다 |
