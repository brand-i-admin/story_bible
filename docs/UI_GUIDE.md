...
13. 프로필 좌측 기록/기도/저장/말씀 컨테이너는 콘텐츠 높이에 맞춰 가변적으로 표현한다. 기록 탭의 감정 통계 칩은 개수순 4열로 나열해 화면 밀도를 줄이고, 정답/오답/헷갈려요 통계는 버튼형 1열 구성 안에서 아이콘+라벨과 문항 수/이야기 수를 2줄로 보여 준다. 통계 버튼은 눌렀을 때 같은 이야기 리스트 컴포넌트에 필터만 바꿔 전달해 관련 이야기를 표시한다.
...
16. 프로필 "나의 다이어리" 탭은 기존 지도형 장식을 쓰지 않고 한국 시간 기준 달력으로 감정 기록을 보여 준다. 접힌 상태에서는 지난주와 이번주 두 줄만 보여 주고, 펼치면 월 전체와 이전/다음 월 이동을 제공한다. 날짜 셀은 얇은 구분선만 사용하고 오늘은 날짜 숫자 동그라미, 선택 날짜는 은은한 배경으로 구분한다. 하루 감정 표시는 한 줄 2개, 최대 2줄이며 3개까지는 아이콘을 노출하고 4개 이상은 두 번째 줄에 `+x`로 남은 개수를 요약한다. 주차 높이는 해당 주에서 가장 많이 기록된 날짜의 표시 줄 수에 따라 가변적으로 잡아 빈 공간을 줄인다. 감정 row 탭은 홈 지도/지역 복원 준비를 기다리지 않고 사건 상세 페이지로 바로 진입하며, 짧은 전환 로딩만 패널 입력을 막는다.
