...
| CompanionDiaryEntriesScreen | `screens/companion_diary_entries_screen.dart` | 프로필 "오늘의 신앙 기록" 탭의 전체보기. 상단에는 아이콘형 이전 버튼과 `오늘의 신앙 기록` 제목을 같은 row에 둔다. 사용자가 작성한 신앙 기록을 최신 날짜순으로 표시하며, 본문 카드는 좌측 버튼 폭에 밀리지 않고 화면 가로 공간을 모두 사용한다. 각 항목은 프로필 탭과 같은 `CompanionDiaryEntryPreviewCard`를 사용해 날짜 → `📝`+제목 → 본문 최대 3줄 순서로 보여 주고, 탭하면 전체 본문 상세 팝업에서 수정/삭제한다. |
| AppPublicationsScreen | `screens/app_publications_screen.dart` | 홈 좌상단 메가폰에서 여는 공지사항/사용법 목록. `ParchmentListPageScaffold`와 프로필 동행 일지 전체보기와 같은 미리보기 카드/상세 팝업 패턴을 사용하며, 상세의 본문 URL 줄 또는 `link_url`은 자동 링크 줄로 렌더링해 `url_launcher`로 외부 브라우저에서 연다. 별도 링크 액션 버튼은 두지 않는다. |
...
| ProfileEmotionDiary | `widgets/profile/profile_emotion_diary.dart` | 프로필 "다이어리" 탭. `user_event_emotion_marks.updated_at`을 KST 날짜로 묶어 접힘 상태에서는 오늘이 포함된 주와 그 전 주, 총 2주(일~토)를 보여 주고, `펼치기`로 해당 월 달력과 이전/다음 월 이동을 제공한다. 날짜 셀은 카드/버튼 테두리 없이 표시하되 날짜 사이에 옅은 그리드 선을 두고, 오늘 날짜 숫자만 원형으로 강조한다. 연도/월 헤더 위에는 감정 카테고리 8개를 1행 8칸으로 표시한다. 달력 아래에는 "오늘의 신앙 기록"과 "오늘의 내 감정"을 다른 프로필 상위 탭과 같은 높이의 아이콘 없는 라벨 전용 선택 탭으로 둔다. 신앙 기록 탭에서는 작성 전 흰색/민트 계열 테두리가 짧게 커졌다 작아지는 + 버튼을 먼저 보여 주고 안내 문구를 그 아래에 배치하며, 작성 후에는 `CompanionDiaryEntryPreviewCard`로 미리보기를 표시한다. 오늘의 내 감정 탭은 선택한 날짜의 감정 row 목록만 표시하고, 감정 row를 누르면 사건 상세 페이지로 바로 진입한다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 활동/진행 섹션을 그린다. 상단 헤더는 작은 `샬롬! 🙌` 인사, 닉네임, `님`, 두 줄 축복 문구를 표시한다. 헤더 아래 `기록/기도/저장/말씀` 활동 섹션과 `다이어리/인물과걷기/장소로시작` 진행 섹션은 하나의 얇은 흰 바깥 패널 안에 들어가며, 좁은 화면에서는 프로필 페이지 전체 스크롤로 `오늘의 신앙 기록` 본문 끝까지 내려간다. 상위 탭들은 작은 단색 아이콘과 라벨을 같은 row에 배치해 높이를 줄이고, 선택 탭에는 팔레트 역할색 배경을 버튼 높이 전체에 표시한다. 달력 아래 `오늘의 신앙 기록/오늘의 내 감정` 탭도 같은 높이의 라벨 전용 선택 탭을 쓴다. 기록 탭 상단은 `통독 진행률` 카드와 `이야기 진행률+정답/오답/헷갈려요` 통합 카드로 나누고, 완료 숫자는 초록 강조색으로 표시한다. 이야기 진행률 팝업은 `전체/완료/미완료` 필터로 시대별 카드 목록을 거른다. 기도 탭의 빈 중보기도 상태와 신앙 기록 빈 상태는 같은 초록 원형 + 버튼을 쓰며, 버튼 바깥 흰색/민트 계열 테두리가 홈 미션처럼 짧게 pulse 되어 입력을 유도한다. 저장/말씀 탭을 누르면 각각 미리보기를 다시 불러오고, 설정 시트는 개인정보 보호, 지도 설명, 로그아웃, 계정 삭제를 제공한다. |
