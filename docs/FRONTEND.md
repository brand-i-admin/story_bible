...
| MapHintOverlay | `widgets/v2/map_hint_overlay.dart` | 지도 위 흐릿한 검정 패널 안내 문구. 상단 배지에 "화면 아무데나 누르면 사라집니다"를 통일 노출해 고정 안내가 아니라 임시 안내임을 드러낸다. 모드별로 다른 메시지: region picker 단계 = "노란 지역을 눌러…", character step 2 = "인물을 골라 「→」 버튼…". 사용자가 지도 제스처/region 선택/character 다음 한 번만 하면 dismiss. `IgnorePointer` 로 감싸 폴리곤·핀 클릭은 그대로 통과. dismiss flag 는 `StoryHomeScreen._mapHintDismissed`. |
