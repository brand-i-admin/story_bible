...
| ProfileLifeMap | `widgets/profile/profile_life_map.dart` | 프로필 "내 삶의 지도" 탭의 감정 새김 지도. `EventEmotionOption`의 8개 감정(기쁨/기대/감사/놀라움/안타까움/위로/두려움/기타)과 지도 영역을 1:1로 맞추고, 각 영역의 최근 새김 수와 최근 남긴 한 줄을 보여 준다. 감정 영역 배지는 아이콘+개수를 하나의 pill 로 표시하며, 영역 탭 시 사건 카드를 Era → storyIndex 순서의 2열 스크롤 그리드로 보여 준다. 최근 한 줄 날짜는 `utils/kst_date.dart`의 KST 달력 날짜 포맷터를 사용해 기기 로컬 시간대에 따라 다음 날로 밀리지 않게 표시한다. |
