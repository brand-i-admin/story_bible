...
`state/color_palette_providers.dart` — `AppColorPalette` enum(`classic` / `atlasNavy` / `colorfulMap` / `blackMap`)과 Riverpod 프로바이더. `classic`은 main 브랜치의 어두운 올리브 chrome과 베이지 패널, `atlasNavy`는 네이비/청록, `colorfulMap`은 파스텔 보라 primary 와 청록·오렌지·핑크 역할색, `blackMap`은 검은 지도 톤과 금빛 포인트를 사용한다. 블랙 팔레트의 `primary`/`primaryDeep`은 어두운 표면색이 아니라 다크 표면 위에서 보이는 청록 액션색이며, `panelSurface`는 불투명한 어두운 표면으로 유지한다. `StoryBibleApp`이 선택된 팔레트로 `AppTheme.light(palette:)`를 다시 만들고, `AppPaletteTheme` extension을 통해 공통 버튼, 홈 상단 유틸리티 버튼, 선택 시트 배경/질감/핸들, 시대·인물·구간 선택 카드, 사건 카드 표면/텍스트, 스텝퍼, 프로필 요약/다이어리/통독/캘린더, 성경 본문 절 번호, 구절 검색 그리드, 알림 패널, 이야기 카드 선택/완료 테두리에 같은 색 조합을 적용한다. 밝은 고정 표면과 다크 팔레트의 밝은 텍스트를 섞지 않도록 `cardSurface`, `softSurface`, `mutedSurface`, `text`, `mutedText`를 함께 사용한다. 프로필의 흰색 쉘은 블랙 팔레트에서 검은 쉘로 바꾸고, 홈 스크롤 선택 패널은 검은 지도 위에서 더 깊은 검정 표면을 쓴다. 저장소는 `data/color_palette_repository.dart`의 `SharedPreferences` 래퍼 사용. 제거된 `brightCoast` 저장값은 `classic`으로 보정된다.
...
| FontScaleBottomSheet | `widgets/font_scale_bottom_sheet.dart` | 색 조합 3개를 1행 3열 버튼으로 보여 주고, `글자 크기` 제목 아래에 성경 구절 미리보기를 둔 뒤 글자 크기 3단계 버튼을 배치하는 바텀시트 + `showFontScaleSheet` 헬퍼. 홈에서 시트가 열린 동안은 `StoryHomeScreen` 이 지도 tap/gesture 를 잠가 색상 버튼 탭과 시트 스크롤이 아래 지도 region·landmark 팝업이나 카메라 이동으로 새지 않게 한다. |
...
| WeeklyTabPage | `widgets/weekly_tab_page.dart` | 주간 미션 탭. 인물/지역 모드 중 하나를 시드로 고르고 지도 + 하단 사건 카드 row 를 보여 준다. 카드 탭은 홈 지도/하단 패널을 해당 사건으로 준비한 뒤 기존 사건 상세로 진입하므로 진행도는 `user_event_progress`/`user_quiz_attempts`/`user_event_emotion_marks`와 그대로 연동된다. |
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 진행 섹션을 그린다. 기도 기능은 `profilePrayerFeaturePending` 상태로 코드/DB 배관을 보존하지만 화면에서는 활동 섹션과 프로필 수정 필드를 노출하지 않는다. 상단 헤더는 `프로필` 제목과 공지사항/사용법, 알림, 설정 아이콘을 표시한다. 진행 섹션은 프로필 정보, `이야기 탐험 요약`, 이야기 카드 덱, `신앙 다이어리`, `통독 진행률`을 하나의 테두리 없는 컨테이너 안에 둔다. 이 컨테이너는 밝은 팔레트에서는 흰색, 블랙 지도에서는 깊은 검정 표면을 써서 내부 다크 카드가 강조되게 한다. 안쪽 표면은 밝기 위계를 둔다: 프로필 정보는 가장 밝은 중립 표면에 아주 옅은 팔레트 tint 를 얹고, `이야기 탐험 요약`~`이야기 탐험` 묶음은 이야기/액션 주황 계열의 옅은 표면, `신앙 다이어리`는 초록 계열, `통독 진행률`은 말씀/primary 계열의 옅은 표면이다. 프로필 영역에는 프로필 사진, `샬롬! 🙌 {닉네임}님` 인사, "오늘도 이야기 탐험, 신앙 다이어리 작성 / 통독으로 하나님과 함께 해보아요!" 문구, `할일:` 뒤의 `이야기 탐험`/`신앙 다이어리`/`통독` 체크박스형 오늘 루틴 상태 표시를 둔다. 상태 표시는 버튼처럼 보이지 않는 낮은 표시이며, KST 기준 감정 새김 1개 이상, 오늘 신앙 다이어리 작성, 오늘 통독 1장 이상 `read_at` 완료 시 체크박스가 초록색 계열로 바뀐다. 할일 row 를 탭하면 프로필 수정 대신 "오늘의 할일" 안내 팝업을 열고, 페이지가 열려 있는 동안 KST 자정이 지나면 오늘 체크 상태와 미리보기를 다시 불러온다. `이야기 탐험 요약`은 4col 카드(`이야기`, `기록`, `저장`, `말씀`)로 보여 주고, 숫자 뒤 `개` 단위는 숫자보다 작게 표시한다. `이야기` 요약은 `완료/전체개` 숫자만 둔다. 요약 카드 색은 현재 팔레트의 `primary`, `currentAccentDeep`, `successBottom`, `primaryDeep` 역할색으로 분산한다. `기록` 카드는 감정 새김 수와 신앙 다이어리 기록 수의 합계를 보여 주고, 누르면 `복습 항목`과 `탐험 달력과 흔적들` 두 묶음의 탐험 로그 페이지를 연다. 카드 덱은 프로필용 compact surface와 텍스트 배율 cap을 사용하고, 메인 카드가 overflow 나지 않도록 세로 여유를 확보한다. 메인 카드만 사건 description/summary를 노출하되 블랙 지도에서는 흰색 대신 다크 카드 표면을 쓰고, `다음 이야기` 카드는 오늘 감정 새김 전까지 내용이 어두워지지 않도록 중심은 거의 투명하고 바깥 edge 가 밝은 노란 gradient/glow 로 깜빡인다. 메인 카드를 누르면 프로필 route 를 닫아 지도에서 해당 번호 핀을 약 1초 pulse 로 보여 준 뒤 상세로 들어가며, 상세 `<`는 지도가 아니라 프로필 페이지로 돌아간다. 덱 아래 안내는 `참고` 라벨이 붙은 테두리 없는 가운데 정렬 pill로 "다음 이야기를 눌러 시작하세요! (완료 조건은 감정 새기기입니다)"를 표시한다. `탐험한 이야기` 페이지는 시대 선택, 전체/완료/미완료 필터, `ProfileEventReviewGrid`만 배치하고 `이야기 진행률` 컨테이너는 팔레트의 밝은 `softSurface`를 사용한다. `저장 이야기`는 저장한 이야기 페이지, `저장한 말씀`은 `SavedVersesScreen`으로 이동한다. 하단에는 바깥 테두리 없는 `신앙 다이어리`와 `통독 진행률` 카드를 같은 높이의 1row 2col 로 배치하고, 완료 전 `기록하기`와 `이어 읽기`는 테두리 없이 느린 퍼짐 pulse 를 계속 반복한다. 감정/다이어리 달력은 프로필 메인이 아니라 `탐험 로그`의 `탐험 달력과 흔적들`에서만 제공한다. 큰 글자 설정에서는 통독 카드의 마지막 통독 장 텍스트를 2줄까지 허용한다. |
...
`ProfileTabPage`는 자체 상태(25+개 필드)를 내부에서 모두 관리하며, 외부 의존성은 콜백 7개로만 노출된다:
