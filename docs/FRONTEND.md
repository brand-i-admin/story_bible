...
| 화면 | 파일 | 역할 |
|------|------|------|
| StoryHomeScreen | `screens/story_home_screen.dart` | 앱 루트 셸. 화면 폭 `StoryRootNavigationBar`로 `오늘`·`성경`·`지도`·`내정보` 4개 탭을 전환한다. `오늘`은 `JourneySelection`으로 운영 DB의 310개 published 이야기를 전체·일부 구간·성경책·인물 범위로 필터링한 뒤 구약 7시대→신약 4시대와 각 시대의 `rankInEra` 순서로 고정한다. 감정 저장 시각은 정렬에 쓰지 않고, 선택 범위에서 감정이 없는 첫 이야기를 오늘의 이야기로 삼으며 전부 완료했으면 마지막 이야기를 유지한다. 헤더 바로 아래 여정 선택 바는 저장된 조건명을 표시하고 `JourneySelectionScreen`을 연다. 현재 시대 전체 번호 핀과 사건 사이 점선, 낮은 `다이어리`/`통독` 액션을 보여 준다. `오늘`과 `지도`는 공유 `GlobalKey`와 `StoryMapPanelController`로 하나의 지도 상태를 재부착해 WebView 재로딩을 피한다. 오늘 지도는 패널 밖에서 팬/줌/회전을 허용하고, 패널 입력 중에만 gesture suspension을 적용한다. 루트 네비게이션 표면은 시스템 하단 영역까지 같은 색으로 이어진다. `지도` 루트 탭은 진입과 이탈 양쪽에서 시대·방법·사건 선택을 비운다. 다이어리/통독은 비로그인 시 `내정보로 이동` 팝업으로 `내정보` 탭을 열며, 로그인 상태에서는 오늘 다이어리 작성·상세·수정·삭제와 다음 통독 위치 이동을 지원한다. 기존 시대·시간순·인물·장소 탐색은 `지도` 탭으로 유지한다. |
| JourneySelectionScreen | `screens/journey_selection_screen.dart`, `screens/journey_selection/` | `전체 순서`를 기본으로 하고 `일부 구간 선택`·`성경책으로 찾기`·`인물로 찾기`를 번호가 붙은 선택 카드로 제공한다. 여정 화면은 장식 없는 `<` 아이콘과 일반 제목을 공통 헤더로 사용한다. 첫 화면의 2열 카드는 아이콘과 제목을 한 줄에 두고 `크게` 글자에서도 `FittedBox.scaleDown`과 가변 설명 높이로 말줄임표 없이 내용을 모두 보여 준다. 밝은 팔레트의 카드 표면은 흰색, 다크는 검은색이다. `전체 순서`는 확인 팝업을 거쳐 적용한다. 선택 카드 안에는 진행 수를 넣지 않고 활성 상태만 표시하며, CTA 위의 `선택된 이야기 정보`는 체크 아이콘·테두리 대신 강조선이 있는 정보 패널로 렌더링한다. 시대 범위 선택이면 패널의 방식명과 완료/전체 수 사이에 실제 선택 소분류 이름을 모두 표시한다. 일부 구간은 구약·신약 필터로 시대를 나누되 필터를 바꿔도 기존 선택을 유지한다. 처음에는 모든 시대를 접고 일부 소분류만 저장된 시대만 다시 펼치며, 시대 전체 선택은 체크된 접힘 상태로 둘 수 있다. 쉬운 시대 제목과 DB 시대명을 같은 한 줄에 두고 긴 조합은 오른쪽 fade가 있는 가로 수동 스크롤로 보여 준다. 실제 `bible_refs`의 성경 권 목록도 정경 순 한 줄 스크롤로 표시하며, 펼친 소분류는 체크박스와 제목을 같은 줄에 둔다. 선택 CTA는 화면 아래에 고정한다. 구간은 실제 `unit_code/unit_title/unit_order`, 권은 실제 `bible_refs`, 인물은 활성 `characters`와 `character_codes/era_codes`로 구성한다. 성경책·인물 목록은 별도 소개 문구 없이 바로 필터를 보여 준다. 공용 고대비 segmented control은 선택 글자를 흰색으로 유지하고, 성경책 목록은 이야기가 없는 권을 중립 회색으로 비활성화하며 선택 권을 강한 강조 표면·체크로 구분한다. 인물은 구약·신약과 단일 정렬 dropdown(`시간순`/`가나다순`)을 한 줄에 두고 5열로 배치한다. 권·인물 상세는 정보 요약과 선택 영역을 다른 표면으로 분리하고 `대상 이야기만`을 먼저, 접힌 `대상이 속한 시대 선택`을 다음에 두며 두 방식 모두 체크박스로 선택한다. 선택 조건은 `JourneySelectionRepository`가 `SharedPreferences`에 저장해 앱 재실행 뒤 복원한다. |
...
| TodayHomePage | `widgets/home/today_home_page.dart` | 조작 가능한 공유 3D 지도 위에 전체 폭 `TodayActivityHeader`, `찾기`·`큰글자`·`테마`, 헤더 바로 아래의 팔레트 기반 `여정 선택` 바, 이야기 덱과 퀵 액션을 조합한다. 여정 바는 `전체 순서`, `일부 구간`, `{권/인물} 이야기만`, `{권/인물} · 시대 구간`처럼 저장된 조건을 표시하며, 오른쪽 끝의 원형 `>` 버튼으로 선택 화면을 연다. 로그인 사용자는 안내 오버레이를 보지 않는다. 비로그인 사용자에게만 `TodayTodoGuide`로 “아래 이야기 카드를 스크롤”하고 카드가 위 여정 선택 기준으로 나열된다는 문구와 `MapHintDismissBadge`를 보여 주며 첫 터치에 닫는다. 현재 이야기 시대의 모든 사건을 번호 핀과 시간순 점선으로 렌더하고 같은 시대의 직전·직후는 `이전`·`다음`, 현재는 썸네일과 `현재` 배지로 구분한다. 지도 핀 번호는 시대 내부 원래 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번을 사용한다. |
| TodayJourneySelectionBar | `widgets/home/today_home_page.dart` | 오늘 헤더 아래에서 현재 여정 조건을 보여 주는 진입 바. 선택 조건은 오른쪽 정렬하고, 28px 원형 `>`를 오른쪽 8px 안쪽 끝에 고정해 남는 공간 없이 선택 화면 진입점을 분명히 한다. 바 전체와 원형 화살표 위치 모두 같은 탭 동작으로 연결된다. |
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 선택된 여정의 이야기를 구약 7시대→신약 4시대와 시대 내 순번으로 고정한 유한 `PageView` 카드 덱이다. 카드 좌상단 번호는 DB의 시대 내부 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번이며 오늘 지도 핀과 일치한다. 현재 카드는 넓고 크게, 이전·다음은 하단선을 맞춘 약 62% 높이로 표시한다. 현재가 첫 이야기일 때 왼쪽 `여정의 시작` 경계 카드도 오른쪽 다음 이야기 카드와 같은 높이·하단선을 사용한다. 추천 위치는 당일 완료 여부와 무관하게 `오늘의 이야기` 라벨을 유지한다. 현재·이전·다음 모두 라벨 다음에 제목을 먼저 두고 썸네일은 2:1 비율로 기존 높이의 약 80%만 사용한다. 감정 새김은 카드 순서를 바꾸지 않는다. 첫·마지막 가상 카드는 `{선택 범위}의 첫/마지막 이야기입니다`를 표시하고, 인접 배지는 `여정 처음/끝`을 쓴다. 현재 카드 위에는 `— 시대명 —` 구분선을, 시대가 바뀌는 인접 카드 사이에는 `{시대명} 이동` 배지를 둔다. 덱 아래에는 44px 높이·최대 156px 폭의 `+ 다이어리 기록`·`→ 통독 이어읽기` 버튼만 가운데 배치한다. |
