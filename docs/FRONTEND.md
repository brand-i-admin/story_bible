...
| CompanionDiaryEntriesScreen | `screens/companion_diary_entries_screen.dart` | 프로필 `신앙 다이어리` 카드/chevron 탭에서 여는 전체보기. `전체`·`이번 달` 필터와 월 기록일/연속일을 보여 준다. 별도 날짜 헤더는 두지 않고, 카드 첫 줄의 제목 오른쪽에 `날짜 · KST 시간`을 표시한다. 항목을 누르면 상세 팝업에서 수정/삭제한다. |
...
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 구약 7시대→신약 3시대와 시대 내 순번으로 전체 이야기를 좌우 `PageView` 카드 덱으로 렌더한다. 현재 카드 크기는 유지하고 양옆 카드와 첫·마지막 탐험 정렬 안내 카드는 가로 10px·상단 38px 여백으로 작게 만들면서 하단선을 현재 카드와 맞춘다. 카드 간격은 이전 값의 약 절반인 3~5px로 둔다. 오늘 KST 기준 탐험 완료 전에는 추천=현재 카드에 `오늘의 이야기`만 표시하고, 하나라도 완료한 뒤에는 추천 위치에서도 `현재 이야기`를 표시한다. 마지막 이야기를 완료한 경우에는 마지막 카드를 현재 위치로 유지한다. 사용자가 다른 카드를 고른 경우에도 `현재 이야기`만 표시하며 현재 카드 위에는 공용 `— 시대명 —` 구분선을 둔다. 시대 변경 `시대명·이동`과 `이전/다음 이야기 없음` 경계 배지는 모두 각 테마의 불투명 `utilitySelectedBackground` 배경, 흰 글자, 강한 현재 강조 테두리·음영으로 카드 위에서 또렷하게 구분한다. 첫·마지막 탐험 정렬 안내는 좁은 실제 기기 폭에서 한글 폰트 줄바꿈으로 고정 높이를 넘으면 아이콘·간격·전체 문구를 함께 비례 축소해 보통/아주큰 글자 모두 RenderFlex 오버플로 없이 표시한다. 세 카드 외곽선은 1px이고 현재 썸네일은 16:9, 양옆은 정사각형이다. 덱 아래 `신앙 다이어리`·`통독 이어읽기` CTA에는 외곽 glow만 유지한다. `+`·`→`는 고정 원형 테두리의 정중앙에 Material 아이콘으로 놓고 바깥 확산 원 애니메이션은 사용하지 않는다. |
...
루트 셸은 성경·내정보 탭을 표시할 때도 직전 `오늘` 또는 `지도` 탭의 `StoryMapPanel` WebView를 뒤쪽 레이어에 장착한 채 유지한다. 숨겨진 동안은 입력과 Flutter 애니메이션만 멈추고, 돌아오면 같은 WebView 인스턴스를 재사용해 3D 지도를 다시 로딩하지 않는다.
