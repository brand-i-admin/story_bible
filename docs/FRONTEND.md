...
| ProfileEventReviewGrid | `widgets/profile/profile_event_review_grid.dart` | 완료·저장한 이야기·복습·감정 필터·구절 찾기 결과가 공유하는 사건 목록. 요약을 숨긴 `StoryEventThumbCard`를 고정 높이 없이 3열로 배치해 카드 아래 빈 공간을 없애고, 첫 era 및 era 변경 지점만 가는 경계선+era 이름으로 표시한다. 화면별 차이는 전달하는 사건 필터뿐이다. |
