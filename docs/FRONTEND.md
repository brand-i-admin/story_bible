...
| TodayJourneySelectionBar | `widgets/home/today_home_page.dart` | 오늘 헤더 아래에서 현재 여정 조건과 감정을 새겨 완료한 이야기 수를 보여 주는 진입 카드. 가장 바깥은 큰 pill 대신 20px radius의 옅은 팔레트 표면과 낮은 그림자를 사용한다. 첫 줄에는 34px 톤형 route 아이콘, `여정 선택`과 현재 조건을 세로로 묶은 정보, 30px 원형 `>`를 배치한다. 둘째 줄에는 작은 `진행률` label과 높이 14px의 단순한 progress track을 두고 `완료/전체` 숫자는 bar 정중앙에 겹쳐 좁은 화면에서도 밖으로 나가지 않게 한다. 바 전체와 원형 화살표 위치 모두 같은 탭 동작으로 연결된다. |
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 선택된 여정의 이야기를 구약 7시대→신약 3시대와 시대 내 순번으로 고정한 유한 `PageView` 카드 덱이다. 카드 좌상단 번호는 DB의 시대 내부 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번이며 오늘 지도 핀과 일치한다. 가운데 카드는 기존보다 약 1.2배 넓히고, 이전·다음은 화면에 약 절반만 보이면서 가운데 카드의 60% 높이가 되도록 배치한다. 인접 카드를 누르면 그 카드가 가운데로 이동하고, 가운데 카드를 다시 눌러야 상세로 진입한다. 현재가 첫 이야기일 때 왼쪽 `여정의 시작` 경계 카드도 오른쪽 다음 이야기 카드와 같은 높이·하단선을 사용한다. 추천 위치의 가운데 정렬 라벨은 당일 완료 여부와 무관하게 `이어볼 이야기`를 유지한다. 라벨·제목·썸네일 사이 간격을 확보하고, 오늘 카드 썸네일은 카드보다 폭이 좁고 높이가 폭의 70%인 가로형으로 표시한다. 가운데 카드만 장소·연대와 인물 라벨 사이에 이야기 요약을 최대 2줄 말줄임표로 표시한다. 카드 높이는 이 구성과 인물 label 전체가 잘리지 않는 최소 높이를 확보하고 인접 카드도 같은 60% 비율로 함께 늘린다. 감정 새김은 카드 순서를 바꾸지 않는다. 첫·마지막 가상 카드는 `{선택 범위}의 첫/마지막 이야기입니다`를 표시하고, 인접 배지는 `여정 처음/끝`을 쓴다. 현재 카드 위에는 `— 시대명 —` 구분선을, 시대가 바뀌는 인접 카드 사이에는 `{시대명} 이동` 배지를 둔다. 오늘 화면에는 다이어리/통독 퀵 액션을 두지 않는다. |
