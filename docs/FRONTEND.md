...
| QuizTabPage | `widgets/quiz/quiz_tab_page.dart` | 홈 상단 "퀴즈" 버튼이 여는 페이지. 두 탭: **매일 퀴즈** (가변 선택지 + 제출 → 도장+별가루 + 해설) + **주간 퀴즈** (embedded WeeklyTabPage). 알림 딥링크는 `/daily-quiz`와 `/weekly`로 초기 탭을 지정할 수 있고, 비로그인 상태에서 주간 퀴즈 탭을 누르면 홈 로그인 유도 스낵바를 함께 띄운다. |
