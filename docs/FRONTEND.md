...
| Character | `models/character.dart` | id, code, name, tagline, description, avatarUrl/avatarStoragePath, displayOrder, eraCodes | 생성자 직접. 전체 활성 인물 조회는 `era_codes`도 읽어 인물 여정이 허용된 시대에서만 사건을 센다. DB 이름이 비어 있거나 code/영어로 내려오면 `data/character_name_fallbacks.dart`의 한글 표시명으로 보정 |
...
| JourneySelection | `models/journey_selection.dart` | 오늘 홈 여정의 출처(`all/segments/book/person`), 범위 방식(`all/units/targetOnly`), 권·인물 식별자, 선택한 `eraId::unitCode` 집합. 이야기 수는 저장하지 않고 현재 DB에서 다시 계산 | `JourneySelection.fromMap()` / `.toMap()` |
...
// journey_selection_providers.dart
journeySelectionProvider        // SharedPreferences에서 복원되는 현재 홈 여정 조건
journeyCatalogProvider          // events_ordered + 11개 시대 + 활성 인물 카탈로그를 병렬 조회해 결합
...
| 화면 | 파일 | 역할 |
|------|------|------|
| StoryHomeScreen | `screens/story_home_screen.dart` | 앱 루트 셸. 화면 폭 `StoryRootNavigationBar`로 `오늘`·`성경`·`지도`·`내정보` 4개 탭을 전환한다. `오늘`은 `JourneySelection`으로 현재 공개된 299개 이야기를 전체·일부 구간·성경책·인물 범위로 필터링한 뒤 구약 7시대→신약 3시대와 각 시대의 `rankInEra` 순서로 고정한다. `era_nt_consummation`과 요한계시록 참조 사건은 검수 완료 전까지 앱에서 한 번 더 제외한다. 감정 저장 시각은 정렬에 쓰지 않고, 선택 범위에서 감정이 없는 첫 이야기를 이어볼 이야기로 삼으며 전부 완료했으면 마지막 이야기를 유지한다. 헤더 바로 아래 여정 선택 바는 저장된 조건명을 표시하고 `JourneySelectionScreen`을 연다. 현재 시대 전체 번호 핀과 사건 사이 점선, 이야기 카드 덱을 보여 주며 다이어리/통독 액션은 `내정보`의 각 요약 카드 안에 둔다. `오늘`과 `지도`는 공유 `GlobalKey`와 `StoryMapPanelController`로 하나의 지도 상태를 재부착해 WebView 재로딩을 피한다. 오늘 지도는 패널 밖에서 팬/줌/회전을 허용하고, 패널 입력 중에만 gesture suspension을 적용한다. 루트 네비게이션 표면은 시스템 하단 영역까지 같은 색으로 이어진다. `지도` 루트 탭은 진입과 이탈 양쪽에서 시대·방법·사건 선택을 비운다. 기존 시대·시간순·인물·장소 탐색은 `지도` 탭으로 유지한다. |
| JourneySelectionScreen | `screens/journey_selection_screen.dart`, `screens/journey_selection/` | 첫 화면은 `어떤 기준으로 이야기를 따라가 볼까요?` 질문 아래에 `성경 전체를 순서대로`·`인물에서 시작하기`·`성경책에서 시작하기`·`시대·구간에서 고르기`를 같은 크기의 세로 카드로 제공한다. 전체 순서는 추천 방식이며, 실제 선택됐을 때 보라 그라데이션과 초록 체크를 표시하고 다른 여정이 선택되면 중립 표면과 `추천` badge를 사용한다. 어느 방식이 선택되어도 같은 선택 표현을 적용한다. 구분선 아래 `현재 선택된 여정` 패널은 단색 route 아이콘과 선택 방식을 표시하고, 세부 범위는 강제 개행 없이 `시대 | 대분류(소분류, 소분류)` 계층으로 묶어 화면 폭에 따라 자연스럽게 감긴다. 권·인물 여정은 대상 이름을 앞에 붙인다. 진행 bar 중앙에는 `완료/전체` 숫자 분수만 겹쳐 표시하고 `완료` 문구는 붙이지 않는다. 이 패널을 누르면 전체 순서는 포함 이야기 수 팝업, 일부 구간은 저장된 선택을 펼치고 해당 위치로 이동한 시대·구간 페이지, 권·인물은 저장된 범위 방식과 체크를 복원한 `읽을 범위 선택` 페이지를 연다. 여정 화면은 장식 없는 `<` 아이콘과 일반 제목을 공통 헤더로 사용하고, 하단 주 CTA는 `선택된 여정으로 지도 위 나열`을 쓴다. `시대·구간에서 고르기` 화면은 구약·신약 필터로 시대를 나누되 필터를 바꿔도 기존 선택을 유지한다. 시대 목록은 시대별 카드가 아니라 하나의 공용 컨테이너 안에 구분선으로 나열한다. 처음에는 모든 시대를 접고 일부 소분류만 저장된 시대만 다시 펼치며, 현재 여정 패널을 통한 복원에서는 선택된 시대를 펼치고 첫 선택 위치로 스크롤한다. 시대 복수 선택 체크박스를 누르면 해당 시대의 소분류를 모두 선택하고 즉시 펼쳐 선택 상태를 보여 준다. 시대 코드 기반 이모지는 별도 열이 아니라 쉬운 시대 제목·DB 시대명과 같은 제목 줄 안에 두고, 펼친 소분류 묶음은 좌우 inset·별도 표면으로 중첩 관계를 보여 준다. 긴 제목 조합은 오른쪽 fade가 있는 가로 수동 스크롤로 보여 준다. 실제 `bible_refs`의 성경 권 목록도 정경 순 한 줄 스크롤로 표시하며, 소분류는 복수 선택 체크박스와 `완료/전체` 숫자 분수를 함께 사용한다. 선택 CTA는 화면 아래에 고정한다. 구간은 실제 `unit_code/unit_title/unit_order`, 권은 실제 `bible_refs`, 인물은 활성 `characters`와 `character_codes/era_codes`로 구성한다. 성경책 목록에서 요한계시록은 `0개`로 비활성화한다. 권·인물 목록 제목은 각각 `성경책에서 시작하기`·`인물에서 시작하기`이며 구약·신약 컨트롤 위 검색창은 입력 즉시 필터링한다. 검색어가 비어 있을 때는 선택된 성약을 따르고, 한 글자라도 입력하면 구약·신약 전체를 대상으로 결과와 개수를 계산한다. 인물은 모든 항목을 감싼 단일 표면 안에 5열로 두고, 셀별 외곽 카드는 없앤다. 반응형으로 약 2.2배 커진 아바타 주위에 완료 도넛을 그리고 위에는 `완료/전체`, 아래에는 이름을 겹쳐 표시한다. 권·인물의 `읽을 범위 선택`은 `대상 이야기만`을 기본 선택된 라디오 카드로 두고 `대상이 속한 시대 선택`은 처음 접힌 미선택 라디오 카드로 둔다. 두 번째 카드를 눌러도 어떤 시대도 자동 체크하지 않으며, 펼치면 대상이 속한 모든 대분류와 소분류를 처음부터 펼친다. 대상 포함 badge는 팔레트의 `characterAccent` 대비 표면·테두리·그림자를 사용한다. 선택 조건은 `JourneySelectionRepository`가 `SharedPreferences`에 저장해 앱 재실행 뒤 복원한다. |
...
| 위젯 | 파일 | 역할 |
|------|------|------|
| TodayActivityHeader | `widgets/home/today_activity_header.dart` | 상태바 안전영역까지 덮는 모서리 없는 전체 폭 헤더다. 하단 `StoryRootNavigationBar`와 같은 팔레트 표면색을 쓰고, `오늘` 제목과 해 아이콘 없이 `샬롬 👋 {닉네임}님,`과 `오늘도 주님과 함께 걸어볼까요!`를 두 줄로 표시한다. 닉네임만 14.2px로 아주 살짝 키우며 비로그인은 `사용자`를 쓴다. |
| TodayActivityLabelRail | `widgets/home/today_activity_header.dart` | `내정보` 탭의 프로필 헤더 아래에 KST 당일 `연속:`·`이야기:`·`다이어리:`·`통독:` 네 라벨을 고정 4열로 둔다. 연속 조건을 채우면 불 아이콘만 더 또렷하게 커졌다 작아지고 은은한 금빛 글로우와 네 방향 반짝임이 반복되며, 밝은 팔레트의 반짝임은 배경 대비가 높은 `primaryDeep`, 다크는 금빛을 사용한다. 이야기·다이어리·통독의 완료 체크는 정적으로 유지한다. 불·이야기는 단색 Material 아이콘, 다이어리·통독은 내정보의 `edit_note`·`menu_book` 아이콘과 맞춘다. 네 활동 아이콘은 `TodayActivityIcons`에 모아 일관된 glyph를 사용한다. 가로 스크롤 없이 축소 적응하며, 연속 라벨은 산정 기준 팝업을 연다. |
| TodayHomePage | `widgets/home/today_home_page.dart` | 조작 가능한 공유 3D 지도 위에 전체 폭 `TodayActivityHeader`, `찾기`·`큰글자`·`테마`, 헤더 바로 아래의 팔레트 기반 `여정 선택` 바와 이야기 덱을 조합한다. 여정 바는 `전체 순서`, `일부 구간`, `{권/인물} 이야기만`, `{권/인물} · 시대 구간`처럼 저장된 조건과 선택 범위의 진행 bar·`완료/전체` 수를 표시하며, 오른쪽 끝의 원형 `>` 버튼으로 선택 화면을 연다. 로그인 사용자는 안내 오버레이를 보지 않는다. 비로그인 사용자에게만 `TodayTodoGuide`로 “아래 이야기 카드를 스크롤”하고 카드가 위 여정 선택 기준으로 나열된다는 문구와 `MapHintDismissBadge`를 보여 주며 첫 터치에 닫는다. 현재 이야기 시대의 모든 사건을 번호 핀과 시간순 점선으로 렌더하고 같은 시대의 직전·직후는 `이전`·`다음`, 현재는 썸네일과 `현재` 배지로 구분한다. 지도 핀 번호는 시대 내부 원래 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번을 사용한다. |
| TodayJourneySelectionBar | `widgets/home/today_home_page.dart` | 오늘 헤더 아래에서 현재 여정 조건과 감정을 새겨 완료한 이야기 수를 보여 주는 진입 카드. 가장 바깥은 큰 pill 대신 20px radius의 옅은 팔레트 표면과 낮은 그림자를 사용한다. 왼쪽에는 34px 톤형 단색 route 아이콘, 오른쪽에는 32px의 차분한 원형 `>`를 두고, 가운데 열은 `여정 선택 · 현재 조건` 아래 4px 간격에 높이 14px progress track을 둔 2행으로 구성해 카드 높이를 줄인다. 화살표 원은 `cardSurface`에 `primary`를 아주 옅게 섞은 표면과 반투명 `primaryDeep` 외곽선·작은 그림자·`primaryDeep` chevron을 사용해 모든 팔레트에서 4.5:1 이상 대비를 유지하면서 과하게 강조하지 않는다. 별도 `진행률` label은 표시하지 않고 `완료/전체` 숫자는 bar 정중앙에 겹친다. 불꽃 이모지는 17px로 키우고 `cardSurface` 원형 헤일로와 얕은 어두운 그림자를 둘러 주황색 채움 위에서도 구분한다. 트랙의 세로 중앙에 두고 불꽃의 가로 중심을 현재 색으로 채워진 구간의 끝점과 정확히 겹친다. 따라서 0%는 왼쪽 끝, 100%는 오른쪽 끝에 불꽃 중심이 놓인다. 트랙은 팔레트별 명시적 채움과 테두리를 사용해 네이비 0% 상태에서도 윤곽이 보인다. 카탈로그 로딩 중에는 불꽃과 잘못된 `0/0` 대신 indeterminate bar와 `불러오는 중`을 표시한다. 바 전체와 원형 화살표 위치 모두 같은 탭 동작으로 연결된다. |
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 선택된 여정의 이야기를 구약 7시대→신약 3시대와 시대 내 순번으로 고정한 유한 `PageView` 카드 덱이다. 카드 좌상단 번호는 DB의 시대 내부 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번이며 오늘 지도 핀과 일치한다. 가운데 카드는 기존보다 약 1.2배 넓히고, 이전·다음은 화면에 약 절반만 보이면서 가운데 카드의 60% 높이가 되도록 배치한다. 인접 카드를 누르면 그 카드가 가운데로 이동하고, 가운데 카드를 다시 눌러야 상세로 진입한다. 현재가 첫 이야기일 때 왼쪽 `여정의 시작` 경계 카드도 오른쪽 다음 이야기 카드와 같은 높이·하단선을 사용한다. 가운데 카드는 별도 상태 라벨과 인물 pill 없이 제목 → 장소·연대 → 썸네일 → 요약 → 팔레트의 `currentAccent`를 사용한 `지금 이어보기` CTA 순서이며, CTA와 카드 본문 모두 같은 이야기 상세 이동을 수행한다. 지도 등 다른 화면의 공용 카드 인물 표시는 유지한다. 오늘 카드 썸네일은 카드보다 폭이 좁고 높이가 폭의 70%인 가로형으로 표시하고 요약은 최대 2줄 말줄임표로 표시한다. 가운데 카드 높이는 글자 배율과 실제 요약 줄 수로 측정한 콘텐츠 높이에 맞추며, 요약 다음에는 최소 토큰 간격만 두고 CTA를 붙이고 CTA 아래에는 `AppSpacing.x2`의 작은 고정 패딩을 둔다. 작게·보통·크게 모두 내부 세로 스크롤 없이 내용을 한눈에 보여 주며, 인접 카드도 측정된 가운데 카드 높이의 60% 비율로 함께 맞춘다. 카탈로그 로딩 중에는 빈 여정 문구 대신 진행 표시를 보여 준다. 감정 새김은 카드 순서를 바꾸지 않는다. 첫·마지막 가상 카드는 `{선택 범위}의 첫/마지막 이야기입니다`를 표시하고, 인접 배지는 `여정 처음/끝`을 쓴다. 현재 카드 위에는 `— 시대명 —` 구분선을, 시대가 바뀌는 인접 카드 사이에는 `{시대명} 이동` 배지를 둔다. 오늘 화면에는 다이어리/통독 퀵 액션을 두지 않는다. |
...
| ParchmentDialog | `widgets/parchment_dialog.dart` | 이야기 상세/공지 상세 등 공용 모달. 표면, 제목, 보조 텍스트, 닫기 버튼, 보조 액션, 입력 필드는 현재 `AppPaletteTheme`를 따라 블랙 팔레트에서도 고정 크림색이 남지 않게 한다. 키보드와 함께 쓰는 긴 입력 팝업은 `flexibleContent`로 본문만 줄여 스크롤하고 하단 액션은 둥근 모달 표면 안에 유지할 수 있다 |
...
| PulseHighlight | `widgets/pulse_highlight.dart` | `active` 가 켜질 때 자식 외곽에 부드러운 glow 를 재생하는 래퍼. `pulseCount`가 있으면 지정한 횟수만 박동한 뒤 숨기고, `null`이면 active 동안 계속 반복한다. EventDetailPage 의 "다음 이야기" 카드와 오늘의 추천 이야기 강조에 사용한다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `내정보` 탭의 진행 화면. 상태바부터 프로필 헤더와 그 아래 KST 당일 활동 라벨 끝까지 하단 루트 네비게이션과 같은 팔레트 표면색을 연속해 사용한다. 밝은 본문 쉘은 활동 라벨과 12px 간격을 두어 라운드 상단이 겹치지 않게 한다. 별도 `내정보` 제목 없이 34px 프로필 사진과 15px 닉네임을 바로 보여 주며, 사진/닉네임 탭은 프로필 수정 팝업을 연다. 블랙 팔레트의 닉네임 입력값은 팔레트 `text`색으로 밝게 표시한다. `이야기 탐험 요약`은 음영으로 구분하고 공개 카탈로그의 실제 합계(현재 299개)를 분모로 사용한다. 공개 전 시대와 요한계시록 참조 사건은 요약·완료·복습 목록에서 제외한다. `복습` 수치는 모든 퀴즈 시도의 오답 문항 수와 헷갈림 문항 수를 합산하며, `완료`를 누르면 제목이 `완료`이고 기본 필터도 `완료`인 페이지를 연다. 전체 사건 카탈로그는 오늘 provider와 공유하고 활성 인물은 한 번만 조회한다. `다이어리`·`통독 진행률` 카드는 기본 최소 높이 118px에서 시작해 하단 액션과 큰 글자 높이에 맞춰 확장한다. 다이어리 `기록하기`는 새 기록 또는 오늘 기록 수정을 열고, 통독 `이어읽기`는 완료되지 않은 다음 장을 성경 리더로 연다. 두 액션은 옅은 색면·pill 라운딩·얇은 외곽선과 작은 원형 기호 배지를 사용하며 반복 glow와 채움 원은 쓰지 않는다. 버튼 위 정보 간격은 유지하고 버튼 아래 여백은 12px로 통일한다. 그 아래 독립 `다이어리` 레이어를 배치하며, 다이어리 바깥 레이어는 테두리 없이 그림자로 구분한다. 비로그인 미리보기는 약한 1.5px 블러와 엷은 덮개로 배경 구성을 식별할 수 있게 하되, 배경 전체는 `IgnorePointer`로 입력을 차단하고 로그인 카드만 작동한다. |
...
iOS는 최소 배포 버전 15.0과 UIScene 생명주기를 사용한다. 플러그인은 CocoaPods
정적 링크로 통합하며, Firebase SwiftPM 바이너리의 공급자 dSYM 누락 경고를 피하도록
프로젝트별 Swift Package Manager 통합은 비활성화한다. `ios/Podfile`과 Runner의
Debug/Profile/Release deployment target은 모두 15.0으로 유지한다.
...
| firebase_core | ^3.15.2 | Firebase 초기화 (FCM) |
| firebase_messaging | ^15.2.10 | FCM 토큰/메시지 — 푸시 알림 |
