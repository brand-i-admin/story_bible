...
| 시대 선택 직후 가이드 | `screens/story_home_screen_state.dart` | `MapHintOverlay`에 `시대명 - 권 이름 목록`과 `선택된 시대를 보는 방법을 선택하세요`를 두 개의 불릿으로 표시한다. 이 단계에서는 가이드 얼굴 PNG와 원형 슬롯을 숨기고 말풍선을 기존 아바타 공간까지 넓힌다. 권 목록은 선택 시대 사건들의 실제 `bible_refs`를 중복 제거하고 66권 정경 순서로 나열한다. 아래 괄호 문구의 `시대 다시 선택` 앞에는 뒤로 화살표, `시대/방법` 앞에는 스테퍼와 같은 집 아이콘을 본문 높이에 맞춘 작은 원형 테두리 안에 표시해 아래 버튼·스테퍼와 연결한다. `혹은 집 아이콘 시대/방법 버튼 클릭`은 둘째 줄에서 따로 시작한다. 새 이야기가 추가되면 별도 고정 문구 수정 없이 권 목록도 함께 갱신된다. |
