...
| PulseHighlight | `widgets/pulse_highlight.dart` | `active` 가 켜질 때 자식 외곽에 1.4s 사이클 골드 glow 를 지정 횟수만 재생하는 래퍼. EventDetailPage 의 "다음 이야기" 카드에 부착해 다음 이동 동선을 짧게 유도하고, active 가 계속 true 여도 무한 반복하지 않는다. |
| EraPickRows | `widgets/v2/era_pick_rows.dart` | 시대 선택 칩 — 구약/신약 두 줄. HomeIntroPanel + ProfileTabPage `탐험한 이야기` 페이지에서 공유한다. 홈 인트로에서는 오른쪽 패널 끝까지 가로 레일을 쓰고, 마지막 칩이 둥근 모서리에 붙지 않도록 trailing scroll padding 을 둔다. 칩 라벨은 코드 기준 짧은 이름(`족장`, `출애굽`, `통일 왕국`, `사도`, `후기 사도` 등)을 쓰고, 검수 전 `era_nt_consummation`은 표시하지 않는다. `eraIconFor(code)` 도 export. |
...
| ProfileEmotionDiary | `widgets/profile/profile_emotion_diary.dart` | 프로필 "다이어리" 탭. `user_event_emotion_marks.updated_at`을 KST 날짜로 묶어 접힘 상태에서는 오늘이 포함된 주와 그 전 주, 총 2주(일~토)를 보여 주고, `펼치기`로 해당 월 달력과 이전/다음 월 이동을 제공한다. 날짜 셀은 카드/버튼 테두리 없이 표시하되 날짜 사이에 옅은 그리드 선을 두고, 오늘 날짜 숫자만 원형으로 강조한다. 진행 섹션에서는 `신앙 다이어리`와 `통독 진행률` 카드를 달력보다 먼저 독립 1row 2col 카드로 보여 주고, 독립 사용 시에는 달력 아래 배치도 지원한다. 두 카드는 팔레트 카드 표면, `AppTextStyles.sectionTitle` 제목, 작은 본문 토큰, 같은 row 높이를 공유한다. `신앙 다이어리` 카드는 단색 아이콘 제목, 우측 chevron, "오늘 하나님과 함께한 순간을 기록해 보세요!" 문구, 가운데 정렬된 pulse `기록하기` 버튼을 제공하며, 버튼의 플러스/라벨은 팔레트 성공 역할색을 따른다. 카드 탭으로 전체 일지 목록을 연다. 오늘 일지가 있으면 `기록하기` 버튼을 숨기고 작성 완료 문구만 보여 준다. `통독 진행률` 카드는 `통독 읽음 처리`로 저장된 완료 장 수만 기준으로 좌측 도넛 진행률과 우측 마지막 통독 완료 장(`권 장`) 및 pulse `이어 읽기` 버튼을 표시한다. 카드 본문을 누르면 마지막 통독 완료 권을 선택한 통독 진행률 표 팝업을 열고, `이어 읽기`는 마지막 완료 장의 다음 장으로 성경 리더를 열며 요한계시록 마지막 장이면 "마지막 장이었습니다." 스낵바를 보여 준다. |
| ProfileCompanionDiary | `widgets/profile/profile_companion_diary.dart` | 프로필 `신앙 다이어리` 카드와 기존 날짜별 일지 작성/상세 팝업을 담당한다. 프로필 feature 카드에서는 카드/chevron 탭으로 전체 일지 목록으로 이동하고, `기록하기`는 선택 날짜의 일지 편집 팝업을 바로 연다. 날짜별 다이어리 섹션에서는 기존 `전체 보기` 버튼을 유지한다. 작성된 일지는 공용 미리보기 카드와 상세 팝업에서 수정/삭제한다. 본문은 1000자까지 입력받는다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 진행 섹션을 그린다. 기도 기능은 `profilePrayerFeaturePending` 상태로 코드/DB 배관을 보존하지만 화면에서는 활동 섹션과 프로필 수정 필드를 노출하지 않는다. 상단 헤더는 `프로필` 제목과 공지사항/사용법, 알림, 설정 아이콘을 표시한다. 진행 섹션 최상단에는 팔레트 기반 밝은 컨테이너 안에 프로필 사진, 작은 `샬롬! 🙌` 인사, 닉네임, `님`, 두 줄 축복 문구, 우측 chevron 수정 진입점을 둔다. 같은 컨테이너 안에서 `이야기 탐험 요약` 4col 카드(`탐험한/이야기`, `탐험/로그`, `저장/이야기`, `저장한/말씀`)와 3장 이야기 카드 덱, 하단 `신앙 다이어리`와 `통독 진행률` 카드까지 이어 보여 준다. 요약 카드는 큰 아이콘, 작은 숫자, 우상단 진입 아이콘, 팔레트 tint/shadow로 탭 가능한 표면을 만든다. `탐험 로그`는 감정 새김 수와 신앙 다이어리 기록 수의 합계를 `x개`로 보여 주고, 누르면 `복습 항목`과 `탐험 달력과 흔적들` 두 묶음의 로그 페이지를 연다. `복습 항목`은 정답/오답/헷갈려요 compact 카드를 결과 표시 영역과 같은 컨테이너 안에 두며, 정답은 비활성화하되 초록색 상태를 유지한다. 오답/헷갈려요는 같은 카드를 다시 누르면 선택이 해제되고, 처음에는 "오답이나 헷갈려요를 누르면 이야기 카드가 나타납니다." 안내를 표시한다. 선택 목록은 `ProfileEventReviewGrid`로 인라인 최대 9개(3줄)까지 보여 주고, 더 있으면 `전체 보기` 바텀시트에서 스크롤 목록을 연다. `탐험 달력과 흔적들`은 감정 카테고리 8개 이모지+라벨+개수 버튼, 구분선, 감정/신앙 다이어리 달력, 선택 날짜 기록 순서의 단일 컨테이너다. 감정 카테고리는 필터가 아니라 누르면 해당 감정 코멘트 전체를 팝업 목록으로 보여 준다. 선택 날짜에 감정 코멘트와 신앙 다이어리가 모두 있으면 감정 코멘트를 카드로 먼저, 신앙 다이어리를 아래 카드로 보여 주고, 둘 다 없으면 "선택한 날짜에 새긴 감정과 코멘트 혹은 신앙 다이어리가 없습니다"를 한 번만 표시한다. 카드 덱은 프로필용 compact surface와 텍스트 배율 cap을 사용한다. 메인 카드만 사건 description/summary를 노출하고 다음 이야기 카드는 내부 빛줄기와 점광을 반복 재생한다. 덱 아래 안내는 `참고` 라벨이 붙은 가운데 정렬 pill로 "다음 이야기를 눌러 시작하세요! (완료 조건은 감정 새기기입니다)"를 표시한다. `탐험한 이야기` 페이지는 시대 선택, 전체/완료/미완료 필터, `ProfileEventReviewGrid`만 배치한다. `저장 이야기`는 저장한 이야기 페이지, `저장한 말씀`은 `SavedVersesScreen`으로 이동한다. 하단에는 팔레트 기반 `신앙 다이어리`와 `통독 진행률` 카드를 같은 높이의 1row 2col 로 배치하고, 감정/다이어리 달력은 프로필 메인이 아니라 `탐험 로그`의 `탐험 달력과 흔적들`에서만 제공한다. 큰 글자 설정에서는 통독 카드의 마지막 통독 장 텍스트를 2줄까지 허용한다. |
