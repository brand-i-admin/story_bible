...
| InlineLoginPromptCard | `widgets/inline_login_prompt_card.dart` | 인라인 소셜 로그인 카드. 카카오/Google/Apple 로그인만 제공하며, 모바일 OAuth는 앱 바깥 기본 브라우저가 아니라 인앱 브라우저/Safari View Controller 계열 launch mode를 사용한다. Apple 네이티브 로그인이 가능한 Apple 기기 앱에서만 Apple 버튼을 추가 노출한다. |
