...
dailyExplorationCatalogProvider // 전체 시대 사건을 오늘 탐험 덫에 공급
...
| `clearMapExplorationSelection()` | 루트 지도 탭 이탈 시 시대·방법·사건 선택만 비우고 사용자 진행 기록은 유지 |
...
| 화면 | 파일 | 역할 |
|------|------|------|
| StoryHomeScreen | `screens/story_home_screen.dart` | 앱 루트 셸. 화면 폭 `StoryRootNavigationBar`로 `오늘`·`성경`·`지도`·`내정보` 4개 탭을 전환한다. `오늘`은 구약 7시대→신약 3시대와 각 시대의 `rankInEra`를 절대 순서로 삼고, 가장 최근 감정 새기기 다음 사건(기록이 없으면 원역사 첫 사건)을 현재 추천으로 삼는다. 현재 시대 전체 번호 핀과 사건 사이 점선, 낮은 `신앙 다이어리`/`통독 이어읽기` 액션을 보여 주며 상단에는 흰색 단색 아이콘의 `찾기`·`큰글자`·`테마`만 둔다. 환영 가이드는 `이야기 탐험`·`신앙 다이어리`·`통독` 세 항목과 최근 감정 다음 추천/`내정보` 기록 안내만 보여 주며, 첫 터치 또는 선택된 `오늘` 탭 재탭으로 닫거나 다시 연다. `오늘`과 `지도`는 공유 `GlobalKey`와 `StoryMapPanelController`로 하나의 지도 상태를 재부착해 WebView 재로딩을 피한다. 오늘 지도는 패널 밖에서 팬/줌/회전을 허용하고, 패널 입력 중에만 gesture suspension을 적용한다. 루트 네비게이션 표면은 시스템 하단 영역까지 같은 색으로 이어진다. `지도` 탭에서 다른 루트 탭으로 이동하면 시대·방법·사건 선택을 비우고, 다시 들어올 때 시대/방법 선택부터 시작한다. 다이어리/통독은 비로그인 시 `프로필로 이동` 팝업으로 `내정보` 탭을 열며, 로그인 상태에서는 오늘 다이어리 작성·상세·수정·삭제와 다음 통독 위치 이동을 지원한다. 기존 시대·시간순·인물·장소 탐색은 `지도` 탭으로 유지한다. |
| BibleVerseSearchScreen | `screens/bible_verse_search_screen.dart` | `오늘` 탭 상단 돋보기에서 여는 구절 기반 이야기 검색 화면. 신약/구약, 권, 장, 절을 고르면 `events_ordered.bible_refs` 범위에 해당 절이 포함된 이야기를 지도와 카드로 보여 주고, 카드 탭은 기존 `EventDetailPage` 흐름으로 이어진다. |
...
| 위젯 | 파일 | 역할 |
|------|------|------|
| TodayHomePage | `widgets/home/today_home_page.dart` | 조작 가능한 공유 3D 지도 위에 48px 터치 영역과 20px 흰색 단색 아이콘의 `찾기`·`큰글자`·`테마` 헤더, 첫 터치에 사라지는 `MapHintOverlay` 환영 가이드, 최소화/최대화 가능한 하단 패널을 조합한다. 환영 가이드는 `환영합니다! 매일 3가지로 주님과 동행해요!` 아래 `① 이야기 탐험`·`② 신앙 다이어리`·`③ 통독`과 두 괄호 참고 줄만 표시하고, 가용 지도 높이가 짧으면 전체를 함께 축소해 overflow를 막는다. 모바일 하단 패널은 지도 탭과 `story_bottom_panel_style.dart`를 공유해 양옆 여백 없이 네비게이션 바에 붙고, 큰 화면에서만 18px 여백을 둔다. 현재 이야기 시대의 모든 사건을 번호 핀과 시간순 점선으로 렌더하고, 같은 시대의 직전·직후는 `이전`·`다음` 라벨, 현재는 핀 안쪽 44px 원으로 자른 첫 장면 data URL 썸네일과 `현재` 배지로 구분한다. 겹친 핀의 우선순위는 `현재 > 다음 > 이전 > 나머지`다. `fitEventIds`에는 같은 시대의 이전·현재·다음만 넘기고 촘촘한 세 사건에는 최대 9.0 줌을 허용해 역할 핀이 가시 지도 영역을 채우게 한다. 전체 시대 핀은 지도 데이터에 유지하며 패널 높이가 바뀌면 새 가림 영역으로 다시 맞춘다. |
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 구약 7시대→신약 3시대와 시대 내 순번으로 전체 이야기를 처음·끝이 있는 좌우 `PageView` 카드 덱으로 렌더한다. 각 카드 폭은 viewport의 32%여서 `이전·현재·다음` 세 장이 한 화면에 모두 보이고, 양옆 카드는 상하 16px씩 줄여 현재 카드보다 낮게 보인다. 카드 본문은 지도 타임라인과 같은 `StoryEventThumbCard`를 쓰며 `todayCurrent`/`todayAdjacent`/`mapTimeline` 표현 모드로 화면별 요약·인물·표면만 분기한다. 시대 경계에만 `{인접 시대명}/이동`, 전체 처음·끝에는 `이전/다음 이야기 없음` 배지와 지도 감정 새기기 기준 정렬 안내 카드를 둔다. `이야기 탐험` 제목 오른쪽에는 카드 탐험 참고 문구를 같은 줄에 두고, 퀵 액션 앞에는 단색 아이콘의 `신앙 다이어리 & 통독` 섹션 제목을 둔다. 고정 화살표 헤더 아래의 카드와 퀵 액션은 세로 스크롤한다. 카드 전환은 현재 시대 지도 핀과 카메라를 함께 갱신하며, 오늘 감정·다이어리·통독이 미완료인 항목에는 `PulseHighlight`를 계속 재생하고 각 완료 시 멈춘다. 낮은 다이어리/통독 퀵 액션과 기존 다이어리 상세·수정·삭제 다이얼로그를 유지한다. |
| StoryRootNavigationBar | `widgets/home/story_root_navigation_bar.dart` | 루트의 `오늘`·`성경`·`지도`·`내정보` 4탭 화면 폭 하단 네비게이션. 외부 여백·둥근 표면·그림자를 쓰지 않는다. 탭 콘텐츠는 시스템 안전영역 위에 두고, 바깥 표면과 시스템 내비게이션 바 색은 같은 팔레트 색으로 화면 맨 아래까지 이어진다. |
| StoryMapPanel | `widgets/story_map_panel.dart` | 운영 지도는 `StoryTerrain3dMap` + MapLibre GL JS 3D 단일 경로다. Android/iOS 는 네이티브 WebView, Flutter Web 은 `HtmlElementView` iframe 브릿지로 같은 HTML 렌더러를 띄운다. OpenFreeMap Liberty style 과 공개 Terrarium DEM 에 pitch/bearing/exaggeration 을 적용하며, 별도 API key나 지도 배경 선택 환경변수를 받지 않는다. 예전 flutter_map 2D tile/layer/pin 폴백은 제거됐다. `activeLandmarks` 와 `eraRegionLandmarks` 는 MapLibre 내부 GeoJSON source 로 전달되고, country boundary/region polygon/label/path/hit-zone 은 MapLibre layer 로, 사건 숫자·감정 핀과 non-region 랜드마크는 DOM marker 로 그린다. `fitEventIds`가 있으면 지도에는 `events` 전체를 유지하면서 해당 사건 좌표만 카메라 bounds에 사용한다. `fitTightClusterMaxZoom`은 가까운 사건 묶음의 확대 상한을 화면별로 조절하고, `showEventPath`는 reveal 상태와 무관하게 사건 사이 점선을 켠다. `StoryEventMarkerPresentation`은 같은 DOM 핀 렌더러에 `mapTimeline` 또는 `dailyJourney` 표현 모드를 전달한다. 지도 탭은 감정을 주 표식으로 두고 번호를 우측 아래 배지로, 오늘은 번호·이전/현재/다음 역할과 현재 썸네일을 주 표식으로 두고 감정을 보조 배지로 렌더링한다. 하단 선택 패널 위 pointer 입력은 지도 브릿지에 짧은 tap suppression 과 gesture suspension 을 전달해 패널 버튼/스크롤이 아래 지도 region·랜드마크 선택이나 카메라 이동으로 새지 않게 한다. tap suppression 은 오동작 방지를 위해 길게 유지하되 gesture suspension 은 pointer down/move 동안만 짧게 갱신하고 pointer up/cancel 에서 즉시 해제한다. 접힌 하단 시트는 실제 보이는 peek/header 높이만 hit 영역으로 두고, 접기 직후 지도 제스처 suspension 을 즉시 clear 해 위쪽 지도 영역이 바로 drag/pitch 입력을 받게 한다. **`onMapInteraction`** 콜백은 MapLibre 쪽 사용자 제스처 이벤트로 부모의 hint overlay dismiss 를 트리거한다. 이야기 간 이동 glow 는 같은 위치 사건 분산 좌표를 재사용해 현재/목표 사건 핀 중심에 맞춘다. `skipAnimation()`은 ordered reveal 타이머까지 멈추고 모든 핀을 즉시 노출한다. `playEmotionStamp(event, stampLabel)` 은 감정 새김 직후 같은 분산 좌표의 지도 핀 위에 감정 도장을 재생한다. reveal/전환/감정 새김의 중복 실행은 내부 guard 로 막는다. Android WebView 는 Hybrid Composition 으로 생성해 SurfaceProducer/ImageReader 버퍼 경고를 줄이면서 route 전환 때 지도를 트리에서 내리지 않는다. |
...
| FontScaleBottomSheet | `widgets/font_scale_bottom_sheet.dart` | 전역 색 조합과 글자 크기를 바꾸는 바텀시트. 기본 호출은 두 섹션을 함께 보여 주고, `DisplaySettingsSection.theme/font`를 넘기면 `오늘` 헤더의 `테마`/`큰글자` 버튼에 맞는 한 섹션만 보여 준다. 선택은 즉시 provider와 `SharedPreferences`에 반영한다. |
...
| CompanionDiaryEntryPreviewCard | `widgets/profile/companion_diary_entry_card.dart` | 프로필/전체보기에서 공유하는 신앙 다이어리 미리보기 카드와 상세/편집/삭제 다이얼로그 헬퍼. 다크 팔레트에서는 카드 표면과 상세 본문 텍스트를 `AppPaletteTheme`의 어두운 표면/밝은 텍스트로 렌더링한다. 프로필 메인과 `오늘` 퀵 액션이 같은 상세 다이얼로그를 재사용한다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `내정보` 탭의 프로필·진행 화면. 기존 프로필 정보와 `신앙 다이어리`·`통독 진행률` feature 카드는 유지한다. `이야기 탐험 요약`은 `완료`·`복습`·`저장`·`말씀` 4개 카드이며, 종전 3장 이야기 덱 자리에는 감정 카테고리·달력·선택 날짜 코멘트/다이어리로 구성된 `탐험 달력과 흔적들`을 직접 노출한다. `복습`은 퀴즈 오답·헷갈림 항목과 이야기 카드만 보여 주고 탐험 흔적을 중복 표시하지 않는다. `embedded=true`이면 루트 탭용 뒤로가기 버튼을 숨기며, 비로그인 상태에서는 기존 인라인 로그인 UI를 표시한다. |
...
| StoryBottomPanelStyle | `widgets/story_bottom_panel_style.dart` | `오늘`과 `지도` 하단 패널의 팔레트 그라데이션, 위쪽 radius, 테두리, 그림자, 반응형 좌우 여백을 공유한다. |
...
프로필 메인에는 이야기 카드 덱 대신 `탐험 달력과 흔적들`을 두며, 추천 이전·오늘·
다음 이야기 탐험은 루트의 `오늘` 탭에서 담당한다.

`복습` 세부 화면은 퀴즈 오답·헷갈림 항목만 보여 주며, 프로필 메인의
`탐험 달력과 흔적들`을 다시 반복하지 않는다.
