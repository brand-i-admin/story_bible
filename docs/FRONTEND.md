...
> 프로필 탭 2,700+줄이 `ProfileTabPage`로 분리되어 자체 상태 관리 + 콜백 4개로 결합도 최소화.
...
| ProfileEditorDialog | `widgets/profile_editor_dialog.dart` | 프로필(닉네임/사진/기도제목) 수정. 프로필 페이지와 같은 팔레트 기반 표면과 작은 단색 아이콘 배지로 사진/닉네임/기도제목 섹션을 구분하고, 저장 액션은 하단 CTA로 둔다 |
...
| EraPickRows | `widgets/v2/era_pick_rows.dart` | 시대 선택 칩 — 구약/신약 두 줄. HomeIntroPanel + ProfileTabPage 이야기 진행률 팝업에서 공유한다. 홈 인트로에서는 오른쪽 패널 끝까지 가로 레일을 쓰고, 마지막 칩이 둥근 모서리에 붙지 않도록 trailing scroll padding 을 둔다. 칩 라벨은 코드 기준 짧은 이름(`족장`, `출애굽`, `통일 왕국`, `사도`, `후기 사도` 등)을 쓰고, 검수 전 `era_nt_consummation`은 표시하지 않는다. `eraIconFor(code)` 도 export. |
...
| ProfileEmotionDiary | `widgets/profile/profile_emotion_diary.dart` | 프로필 "다이어리" 탭. `user_event_emotion_marks.updated_at`을 KST 날짜로 묶어 접힘 상태에서는 오늘이 포함된 주와 그 전 주, 총 2주(일~토)를 보여 주고, `펼치기`로 해당 월 달력과 이전/다음 월 이동을 제공한다. 날짜 셀은 카드/버튼 테두리 없이 표시하되 날짜 사이에 옅은 그리드 선을 두고, 오늘 날짜 숫자만 원형으로 강조한다. 달력 아래는 2열 카드 레이아웃으로, 왼쪽 `신앙 다이어리` 카드는 "오늘 하나님과 함께한 순간을 기록해 보세요!" 문구와 pulse `기록하기` 버튼, `카드보기` 버튼을 제공한다. 오늘 일지가 있으면 `기록하기` 버튼을 숨기고 작성 완료 문구만 보여 준다. 오른쪽 `통독 진행률` 카드는 마지막 묵상 구절 참조(`권 장:절`), 작은 퍼센트, 긴 진행 bar, pulse `이어읽기` 버튼을 표시하며, 카드 본문을 누르면 기존 통독 진행률 표 팝업을 연다. |
| ProfileCompanionDiary | `widgets/profile/profile_companion_diary.dart` | 프로필 `신앙 다이어리` 카드와 기존 날짜별 일지 작성/상세 팝업을 담당한다. `카드보기`는 전체 일지 목록으로 이동하고, `기록하기`는 선택 날짜의 일지 편집 팝업을 바로 연다. 작성된 일지는 공용 미리보기 카드와 상세 팝업에서 수정/삭제한다. 본문은 1000자까지 입력받는다. |
...
| ProfileEmotionStats | `widgets/profile/profile_emotion_stats.dart` | 프로필 감정 통계 계산 헬퍼와 공용 칩 row. 감정을 새긴 고유 이야기 수, `EventEmotionOption`별 개수, 감정별 event id 목록을 제공하며, 기록 탭의 `이야기 통계` 카드가 `내가 새긴 감정들` 4x2 그리드와 감정 필터 팝업에 재사용한다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 활동/진행 섹션을 그린다. 상단 헤더는 작은 `샬롬! 🙌` 인사, 닉네임, `님`, 두 줄 축복 문구를 표시한다. 헤더 아래 `기록/기도/저장/말씀` 활동 섹션과 다이어리 진행 섹션은 하나의 얇은 흰 바깥 패널 안에 들어가며, 좁은 화면에서는 프로필 페이지 전체 스크롤로 `신앙 다이어리`와 통독 카드를 끝까지 내려간다. 활동 탭들은 작은 단색 아이콘과 라벨을 같은 row에 배치해 높이를 줄이고, 선택 탭에는 팔레트 역할색 배경을 버튼 높이 전체에 표시한다. 다이어리 섹션은 `신앙 다이어리` 카드와 마지막 묵상 구절 기반 `통독 진행률` 카드를 함께 보여 준다. 기록 탭은 `이야기 탐험` 카드와 `이야기 통계` 카드로 나누며, 마지막 감정 새김 이야기, pulse `다음 이야기` 버튼, `내가 새긴 감정들` 4x2 그리드, 이야기 진행률, 정답/오답/헷갈려요 통계를 배치한다. 이야기 진행률 팝업은 `전체/완료/미완료` 필터로 시대별 카드 목록을 거르고, 감정 그리드는 복수 감정 필터 팝업을 열어 최신순 감정 row를 날짜/시간과 함께 보여 준다. 기도 탭의 빈 중보기도 상태와 신앙 다이어리 빈 상태는 같은 초록 원형 + 버튼을 쓰며, 버튼 바깥 흰색/민트 계열 테두리가 홈 미션처럼 짧게 pulse 되어 입력을 유도한다. 저장/말씀 탭을 누르면 각각 미리보기를 다시 불러오고, 설정 시트는 개인정보 보호, 지도 설명, 로그아웃, 계정 삭제를 제공한다. |
...
`ProfileTabPage`는 자체 상태(25+개 필드)를 내부에서 모두 관리하며, 외부 의존성은 콜백 4개로만 노출된다:
