...
| ProfileEmotionDiary | `widgets/profile/profile_emotion_diary.dart` | 프로필 "다이어리" 탭. `user_event_emotion_marks.updated_at`을 KST 날짜로 묶어 접힘 상태에서는 오늘이 포함된 주와 그 전 주, 총 2주(일~토)를 보여 주고, `펼치기`로 해당 월 달력과 이전/다음 월 이동을 제공한다. 날짜 셀은 카드/버튼 테두리 없이 표시하되 날짜 사이에 옅은 그리드 선을 두고, 오늘 날짜 숫자만 원형으로 강조한다. 진행 섹션에서는 `신앙 다이어리`와 `통독 진행률` 카드를 달력보다 먼저 독립 1row 2col 카드로 보여 주고, 독립 사용 시에는 달력 아래 배치도 지원한다. 두 카드는 현재 `AppPaletteTheme`의 카드 표면과 역할색을 사용한다. `신앙 다이어리` 카드는 단색 아이콘 제목, 우측 chevron, "오늘 하나님과 함께한 순간을 기록해 보세요!" 문구, 가운데 정렬된 테두리 없는 `기록하기` 버튼을 제공하며 플러스는 `palette.successBottom` 톤을 따른다. 오늘 일지가 없으면 `기록하기` 플러스 링이 느리게 계속 퍼져 나가고, 오늘 일지가 있으면 버튼을 숨긴다. 카드 탭으로 전체 일지 목록을 연다. `통독 진행률` 카드는 `통독 읽음 처리`로 저장된 완료 장 수와 `read_at`을 기준으로 좌측의 살짝 작은 도넛 진행률, 우측 마지막 통독 완료 장(`권 장`) 및 테두리 없는 `이어 읽기` 버튼을 표시한다. 오늘 1장 이상 완료 전에는 `이어 읽기` 주변 glow 가 느리게 계속 pulse 하고, 오늘 완료 후에는 pulse 를 멈춘다. 카드 본문을 누르면 마지막 통독 완료 권을 선택한 통독 진행률 표 팝업을 열고, `이어 읽기`는 마지막 완료 장의 다음 장으로 성경 리더를 열며 요한계시록 마지막 장이면 "마지막 장이었습니다." 스낵바를 보여 준다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 진행 섹션을 그린다. 기도 기능은 `profilePrayerFeaturePending` 상태로 코드/DB 배관을 보존하지만 화면에서는 활동 섹션과 프로필 수정 필드를 노출하지 않는다. 상단 헤더는 `프로필` 제목과 공지사항/사용법, 알림, 설정 아이콘을 표시한다. 진행 섹션은 프로필 정보, `이야기 탐험 요약`, 이야기 카드 덱, `신앙 다이어리`, `통독 진행률`을 하나의 테두리 없는 흰 컨테이너 안에 둔다. 프로필 영역과 `이야기 탐험 요약`~`이야기 탐험` 묶음은 각각 테두리 없는 `AppColors.parchmentCard` 배경 블록으로 나눈다. 프로필 영역에는 프로필 사진, `샬롬! 🙌 {닉네임}님` 인사, "오늘도 이야기 탐험, 신앙 다이어리 작성 / 통독으로 하나님과 함께 해보아요!" 문구, `이야기 탐험`/`신앙 다이어리`/`통독` 오늘 루틴 상태 라벨을 둔다. 상태 라벨은 버튼처럼 보이지 않는 낮은 표시이며, KST 기준 감정 새김 1개 이상, 오늘 신앙 다이어리 작성, 오늘 통독 1장 이상 `read_at` 완료 시 초록색 계열로 바뀐다. `이야기 탐험 요약`은 4col 카드(`이야기`, `로그`, `저장`, `말씀`)로 보여 주고, 숫자 뒤 `개` 단위는 숫자보다 작게 표시한다. `이야기` 요약에는 `완료/전체개`와 작은 progress bar 를 둔다. `탐험 로그`는 감정 새김 수와 신앙 다이어리 기록 수의 합계를 보여 주고, 누르면 `복습 항목`과 `탐험 달력과 흔적들` 두 묶음의 로그 페이지를 연다. 카드 덱은 프로필용 compact surface와 텍스트 배율 cap을 사용하고, 메인 카드가 overflow 나지 않도록 세로 여유를 확보한다. 메인 카드만 사건 description/summary를 노출하고 `다음 이야기` 카드는 오늘 감정 새김 전까지 내용이 어두워지지 않도록 중심은 거의 투명하고 바깥 edge 가 밝은 노란 gradient/glow 로 깜빡인다. 덱 아래 안내는 `참고` 라벨이 붙은 가운데 정렬 pill로 "다음 이야기를 눌러 시작하세요! (완료 조건은 감정 새기기입니다)"를 표시한다. `탐험한 이야기` 페이지는 시대 선택, 전체/완료/미완료 필터, `ProfileEventReviewGrid`만 배치한다. `저장 이야기`는 저장한 이야기 페이지, `저장한 말씀`은 `SavedVersesScreen`으로 이동한다. 하단에는 바깥 테두리 없는 `신앙 다이어리`와 `통독 진행률` 카드를 같은 높이의 1row 2col 로 배치하고, 완료 전 `기록하기`와 `이어 읽기`는 테두리 없이 느린 퍼짐 pulse 를 계속 반복한다. 감정/다이어리 달력은 프로필 메인이 아니라 `탐험 로그`의 `탐험 달력과 흔적들`에서만 제공한다. 큰 글자 설정에서는 통독 카드의 마지막 통독 장 텍스트를 2줄까지 허용한다. |
