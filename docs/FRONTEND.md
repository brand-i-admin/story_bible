...
| AppPublicationsScreen | `screens/app_publications_screen.dart` | 홈 좌상단 메가폰에서 여는 공지사항/사용법 목록. `ParchmentListPageScaffold`와 프로필 신앙 다이어리 전체보기와 같은 미리보기 카드/상세 팝업 패턴을 사용하며, 상세의 본문 URL 줄 또는 `link_url`은 자동 링크 줄로 렌더링해 `url_launcher`로 외부 브라우저에서 연다. 별도 링크 액션 버튼은 두지 않는다. |
...
> 프로필 탭 2,700+줄이 `ProfileTabPage`로 분리되어 자체 상태 관리 + 콜백 5개로 결합도 최소화.
...
| ProfileEmotionDiary | `widgets/profile/profile_emotion_diary.dart` | 프로필 "다이어리" 탭. `user_event_emotion_marks.updated_at`을 KST 날짜로 묶어 접힘 상태에서는 오늘이 포함된 주와 그 전 주, 총 2주(일~토)를 보여 주고, `펼치기`로 해당 월 달력과 이전/다음 월 이동을 제공한다. 날짜 셀은 카드/버튼 테두리 없이 표시하되 날짜 사이에 옅은 그리드 선을 두고, 오늘 날짜 숫자만 원형으로 강조한다. 진행 섹션에서는 `신앙 다이어리`와 `통독 진행률` 카드를 달력보다 먼저 독립 1row 2col 카드로 보여 주고, 독립 사용 시에는 달력 아래 배치도 지원한다. 두 카드는 밝은 표면, `AppTextStyles.sectionTitle` 제목, 작은 본문 토큰, 같은 row 높이를 공유한다. `신앙 다이어리` 카드는 단색 아이콘 제목, 우측 chevron, "오늘 하나님과 함께한 순간을 기록해 보세요!" 문구, 가운데 정렬된 pulse `기록하기` 버튼을 제공하며, 카드 탭으로 전체 일지 목록을 연다. 오늘 일지가 있으면 `기록하기` 버튼을 숨기고 작성 완료 문구만 보여 준다. `통독 진행률` 카드는 좌측 도넛 진행률과 우측 마지막 묵상 구절의 권/장 참조(`권 장`) 및 pulse `이어 읽기` 버튼을 표시한다. 카드 본문을 누르면 마지막 묵상 권을 선택한 통독 진행률 표 팝업을 열고, `이어 읽기`는 다음 장으로 성경 리더를 열며 요한계시록 마지막 장이면 "마지막 장이었습니다." 스낵바를 보여 준다. |
...
| ProfileEventReviewGrid | `widgets/profile/profile_event_review_grid.dart` | 프로필 복습 사건 목록 공용 그리드. `StoryEventThumbCard`를 3열로 배치하고 첫 era 및 era 변경 지점만 가는 경계선+era 이름으로 표시한다. `탐험한 이야기` 페이지와 오답/헷갈려요 복습 팝업이 공유한다. |
...
| ProfileTabPage | `widgets/profile_tab_page.dart` | `UsersRepository`, `ProfilesRepository`, `PrayerRequestsRepository`, `SavedVersesRepository`, `StoryProgressRepository`와 프로필/진행도 provider를 사용해 프로필 헤더와 진행 섹션을 그린다. 기도 기능은 `profilePrayerFeaturePending` 상태로 코드/DB 배관을 보존하지만 화면에서는 활동 섹션과 프로필 수정 필드를 노출하지 않는다. 상단 헤더는 작은 `샬롬! 🙌` 인사, 닉네임, `님`, 두 줄 축복 문구를 표시한다. 헤더 아래 밝은 `이야기 탐험` 컨테이너는 먼저 그래프 단색 아이콘이 붙은 `이야기 탐험 요약` 3col 카드(`탐험한 이야기`, `저장 이야기 개수`, `저장한 말씀`)를 흰색 버튼 표면으로 보여 준다. `탐험한 이야기`는 `x/301` 숫자만 보여 주고 누르면 새 `탐험한 이야기` 페이지를 연다. 요약 아래에는 감정 새김까지 끝난 최신 이야기와 다음 이야기를 기준으로 3장 카드 덱을 구성하고, `다음 이야기` 라벨 카드 자체에 노란 glow 를 짧게 적용한다. 좌우 카드 탭 또는 스와이프로 메인 카드를 바꾸며, 메인 카드를 한 번 더 누르면 해당 이야기 상세로 이동한다. 카드덱은 프로필용 compact surface와 텍스트 배율 cap을 사용해 좁은 폭에서도 좌우 overflow 를 피하고, 메인 카드 제목은 `아주크게` 글자 설정을 따라 커지도록 1.4 배율까지 허용하며, 카드 내용 높이에 맞춘 낮은 덱 높이와 카드 내부 glow 로 하단 빈 공간과 외부 번짐을 줄인다. `탐험한 이야기` 페이지는 프로필형 `<` 뒤로가기와 제목을 쓰며, 맨 위에 테두리 있는 `내가 새긴 감정들` 복수 필터를 둔다. 감정 필터는 큰 글자 설정에서도 가로 스크롤되는 2줄 구조(첫 줄 `전체`+감정 4개, 둘째 줄 감정 4개)를 유지하고, 개수는 각 감정 이모지 우측 아래 배지에 `+n`으로 표시한다. 같은 컨테이너의 구분선 아래에는 최신 감정 코멘트를 월/일만 붙여 최대 3개 보여 주고 더 있으면 `...`와 `전체보기` 버튼으로 전체 감정 목록 페이지를 연다. 감정 row나 진행률 그리드에서 이야기 상세를 열면 `탐험한 이야기` 페이지를 스택에 남겨 상세 `<` 뒤로가기가 해당 페이지로 복귀한다. 그 아래에는 옅은 구분선으로 나뉜 `복습 항목`, 시대 선택, 전체/완료/미완료 필터, `ProfileEventReviewGrid`를 배치한다. 복습 항목 카드는 별도 chevron 없이 카드 탭으로 열고 이야기/문항 수를 가운데 정렬한다. `저장 이야기 개수`는 저장한 이야기 페이지, `저장한 말씀`은 `SavedVersesScreen`으로 이동한다. 하단에는 밝은 표면의 `신앙 다이어리`와 `통독 진행률` 카드를 같은 제목/본문 타이포와 같은 높이의 1row 2col 로 배치하고 달력을 이어 보여 준다. 설정 시트는 개인정보 보호, 지도 설명, 로그아웃, 계정 삭제를 제공한다. |
...
`ProfileTabPage`는 자체 상태(25+개 필드)를 내부에서 모두 관리하며, 외부 의존성은 콜백 5개로만 노출된다:
