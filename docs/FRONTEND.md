...
| TodayJourneySelectionBar | `widgets/home/today_home_page.dart` | 오늘 헤더 아래에서 현재 여정 조건과 감정을 새겨 완료한 이야기 수를 보여 주는 진입 카드. 가장 바깥은 큰 pill 대신 20px radius의 옅은 팔레트 표면과 낮은 그림자를 사용한다. 첫 줄에는 34px 톤형 route 아이콘, `여정 선택 · 현재 조건`을 한 줄로 묶은 정보, 30px 원형 `>`를 배치한다. 둘째 줄에는 작은 `진행률` label과 높이 14px의 progress track을 두고 `완료/전체` 숫자는 bar 정중앙에 겹쳐 좁은 화면에서도 밖으로 나가지 않게 한다. 트랙은 팔레트별 명시적 채움과 테두리를 사용해 네이비 0% 상태에서도 윤곽이 보인다. 카탈로그 로딩 중에는 잘못된 `0/0` 대신 indeterminate bar와 `불러오는 중`을 표시한다. 바 전체와 원형 화살표 위치 모두 같은 탭 동작으로 연결된다. |
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 선택된 여정의 이야기를 구약 7시대→신약 3시대와 시대 내 순번으로 고정한 유한 `PageView` 카드 덱이다. 카드 좌상단 번호는 DB의 시대 내부 번호가 아니라 현재 선택된 전체 여정에서의 1부터 시작하는 순번이며 오늘 지도 핀과 일치한다. 가운데 카드는 기존보다 약 1.2배 넓히고, 이전·다음은 화면에 약 절반만 보이면서 가운데 카드의 60% 높이가 되도록 배치한다. 인접 카드를 누르면 그 카드가 가운데로 이동하고, 가운데 카드를 다시 눌러야 상세로 진입한다. 현재가 첫 이야기일 때 왼쪽 `여정의 시작` 경계 카드도 오른쪽 다음 이야기 카드와 같은 높이·하단선을 사용한다. 가운데 카드는 별도 상태 라벨과 인물 pill 없이 제목 → 장소·연대 → 썸네일 → 요약 → 주황색 `지금 이어보기` CTA 순서이며, CTA와 카드 본문 모두 같은 이야기 상세 이동을 수행한다. 지도 등 다른 화면의 공용 카드 인물 표시는 유지한다. 오늘 카드 썸네일은 카드보다 폭이 좁고 높이가 폭의 70%인 가로형으로 표시하고 요약은 최대 2줄 말줄임표로 표시한다. 카드 높이는 CTA 전체와 실기기 한글 폰트 차이를 흡수할 안전 여유를 확보하고 작게·보통에서는 내부 세로 스크롤 없이 CTA를 카드 하단에 고정한다. 크게에서는 긴 내용을 위한 내부 스크롤을 유지하며, 인접 카드도 같은 60% 비율로 함께 늘린다. 카탈로그 로딩 중에는 빈 여정 문구 대신 진행 표시를 보여 준다. 감정 새김은 카드 순서를 바꾸지 않는다. 첫·마지막 가상 카드는 `{선택 범위}의 첫/마지막 이야기입니다`를 표시하고, 인접 배지는 `여정 처음/끝`을 쓴다. 현재 카드 위에는 `— 시대명 —` 구분선을, 시대가 바뀌는 인접 카드 사이에는 `{시대명} 이동` 배지를 둔다. 오늘 화면에는 다이어리/통독 퀵 액션을 두지 않는다. |
