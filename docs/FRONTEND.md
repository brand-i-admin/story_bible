...
| HomeJourneyOverlay | `widgets/home/home_journey_overlay.dart` | 구약 7시대→신약 3시대와 시대 내 순번으로 전체 이야기를 좌우 `PageView` 카드 덱으로 렌더한다. 현재 카드 크기는 유지하고 양옆 카드와 첫·마지막 탐험 정렬 안내 카드는 가로 10px·상단 38px 여백으로 작게 만들면서 하단선을 현재 카드와 맞춘다. 카드 간격은 이전 값의 약 절반인 3~5px로 둔다. 오늘 KST 기준 탐험 완료 전에는 추천=현재 카드에 `오늘의 이야기`만 표시하고, 하나라도 완료한 뒤에는 추천 위치에서도 `현재 이야기`를 표시한다. 마지막 이야기를 완료한 경우에는 마지막 카드를 현재 위치로 유지한다. 사용자가 다른 카드를 고른 경우에도 `현재 이야기`만 표시하며 현재 카드 위에는 공용 `— 시대명 —` 구분선을 둔다. 시대가 변경되는 카드 사이의 `시대명·이동` 배지는 각 테마의 `currentFill` 배경과 강조 테두리로 일반 경계 배지보다 또렷하게 구분한다. 세 카드 외곽선은 1px이고 현재 썸네일은 16:9, 양옆은 정사각형이다. 덱 아래 `신앙 다이어리`·`통독 이어읽기` CTA에는 외곽 glow만 유지한다. `+`·`→`는 고정 원형 테두리의 정중앙에 Material 아이콘으로 놓고 바깥 확산 원 애니메이션은 사용하지 않는다. |
