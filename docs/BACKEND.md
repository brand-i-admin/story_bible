...
```
- 구약 7시대 + 신약 4시대 = DB 카탈로그 총 11시대. `era_monarchy`는 통일왕국, `era_divided_kingdom`은 왕상 11장 이후 남북 분열부터 예루살렘 포위 전후까지의 분열왕국 이야기를 담는다. `era_nt_consummation`과 요한계시록 참조 사건은 row를 삭제하지 않고 `events.status='draft'`로 보존하며, 앱도 `hiddenEraCodes={'era_nt_consummation'}`와 성경 권 공개 정책으로 이중 차단한다. 따라서 사용자에게는 구약 7시대 + 신약 3시대만 보인다.
...
| 메서드 | 쿼리 | 반환 |
|--------|------|------|
| `fetchEras()` | `eras` ORDER BY display_order → `hiddenEraCodes`의 `era_nt_consummation` 제외 | `List<Era>` |
...
| `fetchEventsByEra(eraId)` | `events_ordered` view WHERE era_id ORDER BY rank_in_era → 숨김 시대와 요한계시록 참조 제외 | `List<StoryEvent>` |
| `fetchAllEvents()` | `events_ordered` view ORDER BY global_rank → 숨김 시대와 요한계시록 참조 제외. 오늘/프로필 카탈로그가 공유한다. | `List<StoryEvent>` |
| `fetchEventsForCharacter(personCode)` | `events_ordered` WHERE character_codes @> ARRAY[code] ORDER BY global_rank → 숨김 시대와 요한계시록 참조 제외 | `List<StoryEvent>` |
| `fetchEventsByIds(eventIds)` | `events_ordered` WHERE id IN (...) ORDER BY global_rank → 숨김 시대와 요한계시록 참조 제외 | `List<StoryEvent>` |
| `searchEventsByText(query)` | 전체 `events_ordered` + persons name lookup → 숨김 시대와 요한계시록 참조 제외 → 클라이언트 가중치 검색 | `List<StoryEvent>` (상위 20) |
...
| `clearBibleVerseHighlight(...)` | 하이라이트 전용 row DELETE 또는 저장 row `highlight_color` null UPDATE | `SavedBibleVerse?` |
