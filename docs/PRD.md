...
고령 사용자의 가독성과 사용자 취향을 위해 앱 전역 색 조합과 글자 크기를 조절할 수 있다. 색 조합은 클래식, 지도 남색, 밝은 해안, 알록 지도 4개이며 글자 크기는 3단계(보통 1.0x / 크게 1.2x / 아주크게 1.4x)이다.

- **접근**: 홈 화면 상단 row의 `색/글자` 버튼 한 곳에서만 변경 (전역 적용되므로 한 곳이면 충분)
- **UI**: 바텀시트에 색 조합 선택, 미리보기, 글자 크기 3단계 버튼. 선택 즉시 전역 반영.
