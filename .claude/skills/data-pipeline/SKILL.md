...
tools/seed/person_meta.json          # 인물 메타 (카탈로그 + 아바타 프롬프트, 생성됨)
assets/                         # 입력/출력 에셋
...
make build-person-meta       # person_meta.json (카탈로그 + 아바타 프롬프트)
make seed-stories            # 이야기 SQL (→ person-meta 의존)
make seed-persons            # 인물 SQL (→ person-meta 의존)
make generate-avatars        # Vertex AI 아바타 (→ person-meta 의존)
