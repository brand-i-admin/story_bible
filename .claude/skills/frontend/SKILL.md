...
- **TDD**: 코드를 고치기 전에 `test/` 에서 관련 테스트를 먼저 찾는다.
  - 새 기능 → 실패 테스트 먼저 작성(Red) → 구현(Green).
  - 기존 기능 수정 → 기존 테스트와 요구사항 비교 → 테스트 수정이 필요하면 `$testing`
    스킬로 넘기고 **사용자 확인 후** 테스트부터 변경(Red) → 구현(Green).
  - 버그 수정 → 버그를 재현하는 실패 테스트부터 작성.
