...
def build_sql(
    person_rows: list[dict[str, Any]], era_rows: list[tuple[str, str, int]]
) -> str:
    lines: list[str] = []
    ...
    if ROSTER_EXCLUDED_CODES:
        for chunk in split_chunks(sorted(ROSTER_EXCLUDED_CODES), 120):
            ...
            lines.append(
                "delete from event_persons ep using persons p "
                f"where ep.person_id = p.id and p.code in ({in_values});"
            )
            lines.append(
                "delete from person_eras pe using persons p "
                f"where pe.person_id = p.id and p.code in ({in_values});"
            )
...
def main() -> int:
    ...
    common_style = str(prompt_data.get("common_style", "")).strip()
