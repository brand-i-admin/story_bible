#!/usr/bin/env python3
"""Build SQL seed for the persons table from person meta + stories.

- persons rows are created from tools/seed/person_meta.json (every individual).
...
- person_eras is no longer materialized here: it is a view derived from
  ``events.person_codes`` (see db_init.sql).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build SQL for persons and person_eras from person meta JSON."
    )
    parser.add_argument(
        "--person-meta-json",
        default="tools/seed/person_meta.json",
        help="Person meta JSON path (code, name, is_active_default, avatar prompt, etc.).",
    )
    parser.add_argument(
        "--stories-dir",
...
def build_sql(person_rows: list[dict[str, Any]]) -> str:
    lines: list[str] = []
    lines.append("-- Generated by tools/seed/build_persons_seed_sql.py")
    lines.append("-- Target table: persons (person_eras is a view; see db_init.sql)")
    lines.append("begin;")
    lines.append("")
    ...
    if ROSTER_EXCLUDED_CODES:
        for chunk in split_chunks(sorted(ROSTER_EXCLUDED_CODES), 120):
            ...
            lines.append(f"delete from persons where code in ({in_values});")
            lines.append("")
    ...
    for chunk in split_chunks(person_rows, 120):
        ...
        lines.append(",\n".join(values))
        lines.append(")")
        lines.append(f"insert into persons ({columns})")
...
def main() -> int:
    ...
    if not meta_path.exists():
        raise FileNotFoundError(f"Person meta JSON not found: {meta_path}")

    meta_data = json.loads(meta_path.read_text(encoding="utf-8"))
    characters = meta_data.get("characters")
    if not isinstance(characters, list):
        raise ValueError(f"Invalid person meta JSON format: {meta_path}")
    ...
    sql_text = build_sql(person_rows=person_rows)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text(sql_text, encoding="utf-8")

    active_count = sum(1 for row in person_rows if row["is_active"])
    print(f"person meta json   : {meta_path}")
    print(f"stories dir        : {stories_dir}")
    print(f"persons total      : {len(person_rows)}")
    print(f"persons is_active  : {active_count}")
