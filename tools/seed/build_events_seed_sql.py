#!/usr/bin/env python3
"""Build SQL seed for the events table from assets/events JSON data.
...
Output directory defaults to: supabase/events
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build seed SQL from assets/events JSON files."
    )
    parser.add_argument(
        "--input-dir",
        default="assets/events",
        help="Directory with canonical event JSON files.",
    )
    parser.add_argument(
        "--output-dir",
        default="supabase/events",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Legacy mapping JSON: (era, story_index) → landmark_code. "
            "Normally unused because assets/events/*.json embeds landmark_code."
        ),
    )
    parser.add_argument(
        "--events-chunk-size",
    ...
    for row in rows:
        ...
        if not isinstance(story_index, int):
            raise ValueError(
                ...
                "Each item in assets/events/*.json must include an integer story_index "
        ...
        landmark_code = normalize_space(str(row.get("landmark_code", "")))
        ...
        if not landmark_code:
            raise ValueError(
                ...
                "assets/events/*.json 항목에 landmark_code 를 추가하세요."
            )
...
def build_seed_sql(
    ...
    *,
    ...
    lines.append("-- Generated by tools/seed/build_events_seed_sql.py")
    ...
    lines.append("-- Upsert events from canonical assets/events JSON")
...
def load_landmark_mapping(path: Path | None) -> dict[tuple[str, int], str]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            ...
            "assets/events/*.json 에 landmark_code 를 넣거나 legacy mapping 경로를 확인하세요."
        )
    data = json.loads(path.read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    landmark_mapping_path = (
        Path(args.landmark_mapping) if args.landmark_mapping else None
    )
    ...
    sql_path = output_dir / "events_seed.sql"
    report_path = output_dir / "events_report.json"
    normalized_path = output_dir / "events_normalized.json"
    ...
    if len(part_chunks) > 1:
        for idx, part_events in enumerate(part_chunks, start=1):
            ...
            part_path = output_dir / f"events_seed_part_{idx:02d}.sql"
